...
if (isset($_POST['imgorchr'])) {
	$USE_IMGORCHR = intval($_POST['imgorchr']);
...
if (isset($_POST['imgdir']) && preg_match('/^[a-zA-Z0-9_\-]+$/', $_POST['imgdir'])) {
	...
	$imgdirchk = true;
	for ($i=0; $i<=9; $i++) {
		if (!file_exists($imgdir.$newdir."/$i.png")) { $imgdirchk = false; break; }
	}
	if ($imgdirchk) {
...
if ($USE_IMGORCHR == 1) {
	...
	$newdir = '';
	$nam = '';
	if (isset($CONF['IMG_DIR']) && ($CONF['IMG_DIR'] != '')) { $newdir = $CONF['IMG_DIR'].'/'; }
	for ($i=0 ; $i<=9 ; $i++) {
		print '<img src="'.$imgurl.$newdir.$i.'.png" border="0" />';
	}
	print	'<br clear="all" />'._LCX_ADM_CHGIMG_NOTE.'</td></tr>';
	if ($CONF['IMG_DIR'] = '') { $mystyle = ' style="background-color:#00FF00"'; }
	else { $mystyle = ''; }
	print	'<form action="imgslct.php" method="post" style="margin-bottom:0;">'.
		...
		'<td class="even">'.
		'<img src="'.$imgurl.'0.png" border="0" alt="0" />'.
		'<img src="'.$imgurl.'1.png" border="0" alt="1" />'.
		'<img src="'.$imgurl.'2.png" border="0" alt="2" />'.
		'<img src="'.$imgurl.'3.png" border="0" alt="3" />'.
		'<img src="'.$imgurl.'4.png" border="0" alt="4" />'.
		'<img src="'.$imgurl.'5.png" border="0" alt="5" />'.
		'<img src="'.$imgurl.'6.png" border="0" alt="6" />'.
		'<img src="'.$imgurl.'7.png" border="0" alt="7" />'.
		'<img src="'.$imgurl.'8.png" border="0" alt="8" />'.
		'<img src="'.$imgurl.'9.png" border="0" alt="9" />'.
	...
	if ($dh = opendir($imgdir0)) {
		while ($nam = readdir($dh)) {
			if ((is_dir($imgdir . $nam)) && ($nam != '.') && ($nam != '..')) {
				if (file_exists($imgdir . $nam.'/0.png')) {
					if ($nam == $CONF['IMG_DIR']) { $mystyle = ' style="background-color:#00FF00"'; }
					else { $mystyle = ''; }
					$imageattr = '';
					if (function_exists('getimagesize')) {
						list($width, $height, $type, $attr) = getimagesize($imgdir.$nam.'/0.png');
						for ($i = 1; $i < 10; $i++) {
							list($width1, $height1, $type1, $attr1) = getimagesize($imgdir.$nam.'/'.$i.'.png');
							if ($width != $width1) { $width = 0; }
							if ($height != $height1) { $width = 0; }
						}
						if ($width != 0) { $imageattr .= ' width="'.$width.'" '; }
						if ($height != 0) { $imageattr .= ' height="'.$height.'" '; }
					}
					print	'<form action="imgslct.php" method="post" style="margin-bottom:0;">'.
						'<tr valign="top" align="left">'.
						'<td class="head">'.
						'<input type="submit" value="'._SELECT.'"'.$mystyle.' />&nbsp;'.$nam.'</td>'.
						'<td class="even">'.
						'<img src="'.$imgurl.$nam.'/0.png" border="0" alt="0"'.$imageattr.'/>'.
						'<img src="'.$imgurl.$nam.'/1.png" border="0" alt="1"'.$imageattr.'/>'.
						'<img src="'.$imgurl.$nam.'/2.png" border="0" alt="2"'.$imageattr.'/>'.
						'<img src="'.$imgurl.$nam.'/3.png" border="0" alt="3"'.$imageattr.'/>'.
						'<img src="'.$imgurl.$nam.'/4.png" border="0" alt="4"'.$imageattr.'/>'.
						'<img src="'.$imgurl.$nam.'/5.png" border="0" alt="5"'.$imageattr.'/>'.
						'<img src="'.$imgurl.$nam.'/6.png" border="0" alt="6"'.$imageattr.'/>'.
						'<img src="'.$imgurl.$nam.'/7.png" border="0" alt="7"'.$imageattr.'/>'.
						'<img src="'.$imgurl.$nam.'/8.png" border="0" alt="8"'.$imageattr.'/>'.
						'<img src="'.$imgurl.$nam.'/9.png" border="0" alt="9"'.$imageattr.'/>'.

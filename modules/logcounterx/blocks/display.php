...
function b_logcounterx_show_counter ($options) {
	...
	while (list($nam, $val) = $xoopsDB->fetchRow($res)) { $CONF[$nam] = $val; }

	if (!isset($CONF['USE_IMGORCHR'])) { $CONF['USE_IMGORCHR'] = 0; }
	if (!isset($CONF['USE_CHR_STYLE'])) { $CONF['USE_CHR_STYLE'] = ''; }
	...
	$thisweek  = date('Y-m-d',	$nowtime - 86400 * intval($dayofweek));
	...
	$imagedir = '';
	if (!isset($CONF['IMG_DIR']) || !preg_match('/^[a-zA-Z_\-]*$/', $CONF['IMG_DIR'])) { $CONF['IMG_DIR'] = ''; }
	if (($CONF['IMG_DIR'] != '') && file_exists(XOOPS_ROOT_PATH.'/modules/logcounterx/images/'.$CONF['IMG_DIR'].'/0.png')) { $imagedir = $CONF['IMG_DIR'].'/'; }
	if (file_exists(XOOPS_ROOT_PATH.'/modules/logcounterx/images/'.$xoopsConfig['theme_set'].'/0.png')) {
		$imagedir = $xoopsConfig['theme_set'].'/';
	}
	$block['imagedir'] = $imagedir;
	$width = 0; $height = 0; $imageattr = '';
	if (function_exists('getimagesize')) {
		list($width, $height, $type, $attr) = getimagesize(XOOPS_ROOT_PATH.'/modules/logcounterx/images/'.$imagedir.'0.png');
		for ($i = 1; $i < 10; $i++) {
			list($width1, $height1, $type1, $attr1) = getimagesize(XOOPS_ROOT_PATH.'/modules/logcounterx/images/'.$imagedir.$i.'.png');

...
<b><font color=blue>初審金額：<input type="text" size="5" name="a135" value= "<? if($a135 =='') echo $a013; else echo $a135;?>" style="border:0px; text-align: right;" readonly >元</font>，辦理親職講座<input type="text" size="5" name="a136" value= "<? if($a136 =='') echo $a014; else echo $a136;?>"/>場。<br></b>
...
<p>
<b>※個別家庭訪視：</b><br />
學校申請個別家庭訪視<font face="Arial, Helvetica, sans-serif" color="#FF0000" size="+1"><? echo $a017;?></font>人次，申請補助經費<font face="Arial, Helvetica, sans-serif" color="#FF0000" size="+1"><? echo number_format($a016);?></font>元(200元/人)。<br />
...
<b>初審金額：個別家庭訪視<input type="text" size="5" name="a139" onchange="js:Count_family(this);"  value="<? echo $a139;?>"/>人次，<input type="text" size="5" name="a138" onChange="change2()" value="<? echo $a138;?>" style="border:0px; text-align: right;" readonly >元。</b>
<p>
<? if($examine == '2') echo '※初審意見：'.'<font face="Arial, Helvetica, sans-serif" color="#FF0000" size="+1">'.$a131.'</font><br>'?>

<b>※審核意見：</b><textarea name="a131" cols="60" rows="2"><? if($examine == '1'){ echo $a131;} else echo $a191;?></textarea>
<p>
<?
//讀取學校檔案資料
...
<script language="JavaScript">
	...
	function toCheck() 
	{
		...
		for(var i = 150 ; i <= 178 ; i+=4)
		{
			if(document.getElementsByName("a" + i)[0].value != "" && document.getElementsByName("a" + i)[0].value != 0 && document.form.a131.value == "")
			{
				alert('請填寫「審核意見」');
				return false;
			}
		...
		if (document.getElementsByName("pass")[0].checked == false && document.getElementsByName("pass")[1].checked == false)
		{
			alert('請選擇「審核結果」');
			return false;
		}
...
<script language="JavaScript">
	...
	{	
		...
		var oldMoneyName = "a" + padLeft((((parseInt(right(obj.name)) - 150) / 4) + 3).toString(),2) + "6";
	...
	function chkbox(obj)
	{
		...
		{
			document.getElementsByName("a186")[0].value = "";
			document.getElementsByName("a187")[0].value = "";
		}
	...
	function count_all()
	{
		...
		//把a036 ~ a106加總，空值跳過
		for(var i = 150 ; i <= 186 ; i+=4)
		{
			if(document.getElementsByName("a" + i)[0].value != "")
			{
				if(!(i == 186 && document.getElementsByName("a122")[0].checked == false))
	...
	function Count_family(obj)
	{
		//驗證輸入的資料是否為數字 
		var regex = /^[0-9]*$/;
		if (!(regex.test(document.getElementsByName("a139")[0].value)))
		{
			alert('「家庭訪視人次」必須為數字');
			document.getElementsByName("a139")[0].value = "";
			document.getElementsByName("a139")[0].focus();
		}
		else
		{
			if (document.getElementsByName("a139")[0].value == "")
				document.getElementsByName("a139")[0].value = 0;
				
			document.getElementsByName("a138")[0].value = parseInt(document.getElementsByName("a139")[0].value) * 200;
			count_all();
		}

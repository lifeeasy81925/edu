...
while($row = mysql_fetch_row($result_school)){
	...
	$s1=array("","經常門","資本門");
	$s2=array("","電腦及周邊器材","攝影及音訊器材","音樂及美術器材","行政事務用品","教學空間用品","運動器材","其他");
...
//補助項目資料	
$sql_allowance = "select * from 102allowance4 where account like '$id'";
...
<p><b>補助項目四：充實學校基本教學設備</b>　<a href="/edu/modules/xSchoolForm/download/allowance-04.htm" target="_blank">(補助項目說明)</a><br />
說明：6班以下最高補助10萬元，7至12班最高補助15萬元，13班以上最高補助20萬元。<br />
</p>
<form name="form" method="post" action="102_examine_allowance4_finish.php?id=<? echo $id;?>" onKeyDown="if(event.keyCode == 13) return false;">
...
●最近三年是否曾獲本項補助：<font face="Arial, Helvetica, sans-serif" color=red size="+1"><? if($a029==1) echo "近三年度曾接受本項補助，不得再提出申請"; else echo "未受補助"; ?></font><p>
學校申請：經費共計<font face="Arial, Helvetica, sans-serif" color=red size="+1"><? echo number_format($a010);?></font>元，其中包含經常門經費<font face="Arial, Helvetica, sans-serif" color=red size="+1"><? echo number_format($a011);?></font>元，資本門經費<font face="Arial, Helvetica, sans-serif" color=red size="+1"><? echo number_format($a012);?></font>元。<br />
<? if($examine == '2') echo '●縣市審核'.'<font face="Arial, Helvetica, sans-serif" color=red size="+1">'.number_format($city1).'</font>元(經常門<font face="Arial, Helvetica, sans-serif" color=red size="+1">'.number_format($city1_1).'</font>元，資本門<font face="Arial, Helvetica, sans-serif" color=red size="+1">'.number_format($city1_2).'</font>元)'?>
<b>初審金額：<font color=blue>經費共計<input type="text" size="6" name="a132" value="<? if($a132=='') echo $a010; else echo $a132;?>" style="border:0px; text-align: right;" readonly>元，其中包含經常門經費<input type="text" size="6" name="a133" onChange="change1()" value="<? if($a133=='') echo $a011; else echo $a133;?>" style="border:0px; text-align: right;" readonly >元，資本門經費<input type="text" size="6" name="a134" onChange="change2()" value="<? if($a134=='') echo $a012; else echo $a134;?>" style="border:0px; text-align: right;" readonly >元。</font></b>
<? //經費表 充實學校基本教學設備 ?>
<table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFCC" style="font-size:12px;">
  <tr>
    <td colspan="9" align="center" nowrap="nowrap" bgcolor="#99CC66"  style="font-size:14px;">充實學校基本教學設備經費概算：學校申請</td>
  ...
  <tr>
    ...
    <select name="a031" id="s1_2" size="1">
      ...
      <option <? if($a031==$s2[6]) echo "selected"; ?>><? echo $s2[6]; ?></option>
      <option <? if($a031==$s2[7]) echo "selected"; ?>><? echo $s2[7]; ?></option>
    </select>
    </td>
...
//列出狀態訊息
echo "<b>※應上傳檔案：</b><br>";
if ($a4_1 == ''){echo "‧實施計畫：<font color=red>未上傳</font>";} else {echo '‧實施計畫：<font color=blue>已上傳</font>，檔名：'.'<a href="../xSchoolForm/upload/102/'.$school.'/'.$a4_1.'" style="text-decoration:none" target="_new">'.$a4_1.'</a>';}
?>
<? if($p149=="") echo "<!--"; ?>
...
<script language="JavaScript">
	...
	function count_all(group_ab)
	{
		...
		{
			//
			document.getElementsByName("a132")[0].value = total;	
			document.getElementsByName("a133")[0].value = money_1;	
			document.getElementsByName("a134")[0].value = money_2;	
		}

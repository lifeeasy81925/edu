...
while($row = mysql_fetch_row($result_allowance)){
	...
	$a133 = $row[133];	//本項目縣市初審修建
	$a134 = $row[134];	//本項目縣市初審設備
	$a135 = $row[135];	//
	$a136 = $row[136];	//
	$a137 = $row[137];	//
	$a138 = $row[138];	//
...
<? if($examine == '2') echo '●縣市審核'.'<font face="Arial, Helvetica, sans-serif" color=red size="+1">'.number_format($city1).'</font>元(修建<font face="Arial, Helvetica, sans-serif" color=red size="+1">'.number_format($city1_1).'</font>元，設備<font face="Arial, Helvetica, sans-serif" color=red size="+1">'.number_format($city1_2).'</font>元)'?>
<b>初審金額：<font color=blue>經費共計<input type="text" size="6" name="a132" value="<? if($a132=='') echo $a010; else echo $a132;?>" style="border:0px; text-align: right;" readonly>元，其中包含修建<input type="text" size="6" name="a133" onChange="change1()" value="<? if($a133=='') echo $a011; else echo $a133;?>" style="border:0px; text-align: right;" readonly >元，設備<input type="text" size="6" name="a134" onChange="change2()" value="<? if($a134=='') echo $a012; else echo $a134;?>" style="border:0px; text-align: right;" readonly >元。</font></b>
<? //經費表 補助項目七 ?>
...
<script language="JavaScript">
	...
	function count_all(group_ab)
	{
		...
		if(group_ab == "a")
		{
			...
			document.getElementsByName("a133")[0].value = fix;	
			document.getElementsByName("a134")[0].value = Equipment;	
		}

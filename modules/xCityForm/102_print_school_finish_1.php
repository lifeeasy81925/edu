...
<table border="1" cellpadding="0" cellspacing="0">
	<tr align="center">
		...
		-->
<? 
$sql = "select 102school_upload_name.*, 102schooldata.* from 102school_upload_name INNER JOIN 102schooldata ON 102school_upload_name.account=102schooldata.account where `cityname` like '$city'";
...
while($row = mysql_fetch_row($result)) {
	...
	if($row[76] == "$city" && $row[75] == '1'){
		if($row[38] != "" && $row[39] != ""){
			...
			/*1040510修改，隱藏下載路徑
			echo "<td>";
			echo "<a href='/epa_uploads/102/pub/".$row[0]."/".$row[38]."' target='_new'>".$row[38];//檔案一
			//echo "<a href='../xSchoolForm/upload/102/".$row[0]."/".$row[38]."' target='_new'>".$row[38];//檔案一			
			echo "</td>";
			echo "<td>";
			echo "<a href='/epa_uploads/102/pub/".$row[0]."/".$row[39]."' target='_new'>".$row[39];//檔案二

...
<font color=blue>※補助發展原住民文化特色及設備器材-初審金額：<input type="text" size="6" name="city_total_money" value="<?=$city_total_money;?>" style="border:0px; text-align: right;" readonly >
元 (本列自動計算)</font>
<p>
<table border="0">
	<tr>
...
<script language="JavaScript">
	...
	{		
		...
		for(i = 1;i <= 2;i++)
		{
			var city_p_money = document.getElementsByName('city_p' + i + '_money')[0];
			if(city_p_money != null)
			{
				...
				for(j = 1;j <= 10;j++) //計算各特色經常 & 資本，
				{
					...
					if(city_money != null)
					{
						if(opt_category.value == '加班費' && city_money.value != '')
							overtime = parseInt(overtime) + parseInt(city_money.value);
					}
				}
				...
				var diff = parseInt(city_p_money.value) - parseInt(overtime);

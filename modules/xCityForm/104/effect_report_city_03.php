...
<table width="100%" border="1" cellspacing="0" cellpadding="0">
	<tr>
		<td height="60" colspan="23" align="center"><strong><font color=blue>教育部 <?=$school_year;?> 年度推動教育優先區計畫直轄市、縣(市)政府各補助項目執行成果一覽表<br />
	...
	<tr>
		...
		<td colspan="9" align="center" bgcolor="#FFFFCC">教育部核定項目、數量及金額</td>
		<td colspan="9" align="center" bgcolor="#FFFFCC">學校執行成果</td>
		<td rowspan="4"  align="center" bgcolor="#FFFFCC">執行成效百分比(%)</td>
	</tr>
	<tr>
		<td colspan="4" align="center" bgcolor="#FFFFCC">教師宿舍</td>
		<td colspan="4" align="center" bgcolor="#FFFFCC">學生宿舍</td>
		<td rowspan="3" align="center" bgcolor="#FFFFCC" width="50">合計</td>
...
<?
	...
	$sql = " select sd.account as sd_account, sd.schooltype, sd.cityname, sd.district, sd.schoolname, sd.area, sd.getmoney_85to91, sd.traffic_status ".
		   ...
		   "                       left join $a3_table_name_effect as a3_e on a3.sy_seq = a3_e.sy_seq ".
	...
	while($row = mysql_fetch_array($result))
	{
		...
		// 總計
		$sum_edu_p1_current_cnt   += $edu_p1_current_cnt;
		$sum_edu_p1_current_money += $edu_p1_current_money;
		$sum_edu_p1_capital_cnt   += $edu_p1_capital_cnt;
		$sum_edu_p1_capital_money += $edu_p1_capital_money;

		$sum_edu_p2_current_cnt   += $edu_p2_current_cnt;
		$sum_edu_p2_current_money += $edu_p2_current_money;
		$sum_edu_p2_capital_cnt   += $edu_p2_capital_cnt;
		$sum_edu_p2_capital_money += $edu_p2_capital_money;

		$sum_p1_current_cnt    += $p1_current_cnt;
		$sum_p1_current_money  += $p1_current_money;
		$sum_p1_capital_cnt    += $p1_capital_cnt;
		$sum_p1_capital_money  += $p1_capital_money;

		$sum_p2_current_cnt   += $p2_current_cnt;
		$sum_p2_current_money += $p2_current_money;
		$sum_p2_capital_cnt   += $p2_capital_cnt;
		$sum_p2_capital_money += $p2_capital_money;
		...
		if($edu_total_money > 0)
		{
			...
			?>
				<tr>
					...
					<td height="30" align="left"><?=$district.$schoolname;?></td>
					...
					<td align="right"><?=number_format($p1_current_money);?></td>
...
?>
	<tr bgcolor="#FFFFCC">
		...
		<td align="right"><?=number_format($sum_edu_p1_current_cnt);?></td>
		<td align="right"><?=number_format($sum_edu_p1_current_money);?></td>
		<td align="right"><?=number_format($sum_edu_p1_capital_cnt);?></td>
		<td align="right"><?=number_format($sum_edu_p1_capital_money);?></td>
		<td align="right"><?=number_format($sum_edu_p2_current_cnt);?></td>
		<td align="right"><?=number_format($sum_edu_p2_current_money);?></td>
		<td align="right"><?=number_format($sum_edu_p2_capital_cnt);?></td>
		<td align="right"><?=number_format($sum_edu_p2_capital_money);?></td>
		<td align="right"><?=number_format($sum_edu_total_money);?></td>

		<td align="right"><?=number_format($sum_p1_current_cnt);?></td>
		<td align="right"><?=number_format($sum_p1_current_money);?></td>
		<td align="right"><?=number_format($sum_p1_capital_cnt);?></td>
		<td align="right"><?=number_format($sum_p1_capital_money);?></td>
		<td align="right"><?=number_format($sum_p2_current_cnt);?></td>
		<td align="right"><?=number_format($sum_p2_current_money);?></td>
		<td align="right"><?=number_format($sum_p2_capital_cnt);?></td>
		<td align="right"><?=number_format($sum_p2_capital_money);?></td>
	...
	<tr>
		<td height="40" colspan="23">承辦人：　　　　　　　　　　　　　　　　　　　　主管科長：　　　　　　　　　　　　　　　　　　　　局 (處) 長：</td>
	</tr>
</table>
</div>
<?php

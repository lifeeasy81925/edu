...
<?
	...
	//上傳路徑
	$upload_path = $_SERVER['DOCUMENT_ROOT'].'/edu_upload/'.$school_year.'/'.$username.'/';
	
	//建立edu_upload 資料夾
	if(!is_dir($_SERVER['DOCUMENT_ROOT'].'/edu_upload'))
	{	
		//echo "<br />建立目錄:".$_SERVER['DOCUMENT_ROOT'].'/edu_upload<br />';
		//$msg[0] = "建立目錄:".$_SERVER['DOCUMENT_ROOT']."/edu_upload \n";	
		mkdir($_SERVER['DOCUMENT_ROOT'].'/edu_upload', 0777);
	}
	//建立school_year 資料夾
	if(!is_dir($_SERVER['DOCUMENT_ROOT'].'/edu_upload/'.$school_year)) 
	{	
		//echo "<br />建立目錄:".$_SERVER['DOCUMENT_ROOT'].'/edu_upload/'.$school_year.'<br />';
		//$msg[0] = "建立目錄:".$_SERVER['DOCUMENT_ROOT']."/edu_upload/".$school_year." \n";	
		mkdir($_SERVER['DOCUMENT_ROOT'].'/edu_upload/'.$school_year, 0777);
	
	}
	//建立account 資料夾
	if(!is_dir($_SERVER['DOCUMENT_ROOT'].'/edu_upload/'.$school_year.'/'.$username)) 
	{	
		//echo "<br />建立目錄:".$_SERVER['DOCUMENT_ROOT'].'/edu_upload/'.$school_year.'/'.$username.'<br />';
		//$msg[0] = "建立目錄:".$_SERVER['DOCUMENT_ROOT']."/edu_upload/".$school_year.'/'.$username." \n";	
		mkdir($_SERVER['DOCUMENT_ROOT'].'/edu_upload/'.$school_year.'/'.$username, 0777);
	}
	...
	if ($_FILES["myfile"]["size"] < 104857600) //檢查檔案大小
	{
		...
		else
		{
			...
			if (move_uploaded_file($_FILES["myfile"]["tmp_name"], $upload_path . $_FILES["myfile"]["name"]))
			{
				...
				chmod($_SERVER['DOCUMENT_ROOT'].'/edu_upload/'.$school_year.'/'.$username.'/'.$filename, 0777); //chmod 在 windows 下沒用

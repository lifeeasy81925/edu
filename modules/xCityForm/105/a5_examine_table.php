...
<br />補助項目五：發展原住民教育文化特色及充實設備器材<br /><br />
已申請學校列表：
...
<?
	...
	$sql = " select sd.account as sd_account, sd.schooltype, sd.cityname, sd.district, sd.schoolname, sd.area, sd.getmoney_85to91, sd.traffic_status ".
		   ...
		   //j10407，新增 
		   "      , sun_ly.effect_a5_1 as effect_a5_1_ly, sun_ly.effect_a5_2 as effect_a5_2_ly  ".		  		   
	...
	while($row = mysql_fetch_array($result))
	{
		...
		//j10407，新增
		$effect_a5_1 = $row['effect_a5_1_ly'];
		$effect_a5_2 = $row['effect_a5_2_ly'];		
		...
		if($s_total_money > 0 && in_array($a_num,$applied_ary) && in_array($a_num,$can_apply_ary))
		{
			...
			//echo "<form action='examine_allowance_$a_num.php' method='post' style='margin:0px; display:inline;'>";  //j10407，原本的
			
			echo ($inherit_year == '104')?"<form action='examine_allowance_a5_ctn.php' method='post' style='margin:0px; display:inline;'>":"<form action='examine_allowance_$a_num.php' method='post' style='margin:0px; display:inline;'>";          //j10407，修改，判別是否沿用，若沿用，無須再審//			
			...
			//echo ($inherit_year == '104')?"<font size=2 color=red>今年度免審</font><br>".get_url($effect_a5_1, $file_path, "<font size=1 color=blue>查看104年度<br>修正後計畫</font>"):"";    //j10407，新增，判別是否沿用，若為104，則為沿用<img src='/edu/images/ok1.jpg' width='18' height='18' >	
            echo ($inherit_year == '104')?"<font size=2 color=red>今年度免審</font><br><a href='../104/effect_view_school_up_05.php?id=".$account."' target='_blank'><font size=1 color=blue>查看104年度<br>修正後計畫</font></a>":"";			

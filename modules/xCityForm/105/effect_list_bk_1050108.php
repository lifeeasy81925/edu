...
<?
	...
	$sql = " select sd.account, sd.schooltype, sd.cityname, sd.district, sd.schoolname, sd.area, sd.getmoney_85to91, sd.traffic_status ".
		   ...
		   "      , a2.edu_p2_money as a2_edu_p2_money ".  // 105.01.08加入特色二複審金額   
		   "      , a2.inherit_year as a2_inherit_year ". // 105.01.07加入沿用年
		   ...
		   "      , a5.edu_p2_money as a5_edu_p2_money ".  // 105.01.08加入特色二複審金額 
		   "      , a5.inherit_year as a5_inherit_year ". // 105.01.07加入沿用年
	...
	while($row = mysql_fetch_array($result))
	{
		...
		// 105.01.07，新增，判別是否沿用，inherit為沿用自哪一年(如：104)。
		...
		//105.01.08新增，補助二補助五加入特色二的複審金額，用來判斷要不要秀出特色二。
				...
				?>
					...
					<tr>
						<td colspan="4" align="left">
							<font color="#8000FA">原計畫：</font><br />
							<?  
								if (strcasecmp($a2_inherit_year, $school_year-1) == 0) // 沿用自前1年度的計畫，排除change情況。105.01.07修改
								{
									echo "特色一實施計畫：沿用".$a2_inherit_year."年度計畫。<br>";
									if($a2_edu_p2_money > 0) echo "特色二實施計畫：沿用".$a2_inherit_year."年度計畫。<br>";  // 特色二有錢才顯示。105.01.08新增
									
									// 這是附連結的按鈕，先做好預備著，有需要才開啟，不需要就隱藏放著就好。
									// echo ("<a  href=\"../".$school_year-1."/effect_view_school_up_02.php?id=".$account." \"target=\"_blank\">沿用".$a2_inherit_year."年度計畫</a>");
								}
								else if($a2_inherit_year == $school_year-2) // 沿用自前2年度的計畫，排除change情況。。105.01.07修改
								{
									echo "特色一實施計畫：沿用".$a2_inherit_year."年度計畫。<br>";
									if($a2_edu_p2_money > 0) echo "特色二實施計畫：沿用".$a2_inherit_year."年度計畫。<br>";  // 特色二有錢才顯示。105.01.08新增
									
									// 功能同上。
									// echo ("<a  href=\"../".$school_year-2."/effect_view_school_up_02.php?id=".$account." \"target=\"_blank\">沿用".$a2_inherit_year."年度計畫</a>");
								}								
								else // 其餘情形就是當年度計畫，包含change情況。105.01.07修改
								{
									echo get_url($a2_1, $file_path, "特色一實施計畫")."<br>";
									if($a2_edu_p2_money > 0) echo get_url($a2_2, $file_path, "特色二實施計畫")."<br>";  // 特色二有錢才顯示。105.01.08新增
								}
							?>
						</td>
					</tr>
				...
				?>
					...
					<tr>
						<td colspan="4" align="left">
							<font color="#8000FA">原計畫：</font><br />
							<? 
								if (strcasecmp($a5_inherit_year, $school_year-1) == 0) // 沿用自前1年度的計畫，排除change情況。105.01.07修改
								{
									echo "特色一實施計畫：沿用".$a5_inherit_year."年度計畫。<br>";
									if($a5_edu_p2_money > 0) echo "特色二實施計畫：沿用".$a5_inherit_year."年度計畫。<br>";  // 特色二有錢才顯示。105.01.08新增
									
									// 這是附連結的按鈕，先做好預備著，有需要才開啟，不需要就隱藏放著就好。
									// echo ("<a  href=\"../".$school_year-1."/effect_view_school_up_02.php?id=".$account." \"target=\"_blank\">沿用".$a5_inherit_year."年度計畫</a>");
								}
								else if($a5_inherit_year == $school_year-2) // 沿用自前2年度的計畫，排除change情況。。105.01.07修改
								{
									echo "特色一實施計畫：沿用".$a5_inherit_year."年度計畫。<br>";
									if($a5_edu_p2_money > 0) echo "特色二實施計畫：沿用".$a5_inherit_year."年度計畫。<br>";  // 特色二有錢才顯示。105.01.08新增
									
									// 功能同上。
									// echo ("<a  href=\"../".$school_year-2."/effect_view_school_up_02.php?id=".$account." \"target=\"_blank\">沿用".$a5_inherit_year."年度計畫</a>");
								}								
								else // 其餘情形就是當年度計畫，包含change情況。105.01.07修改
								{
									echo get_url($a5_1, $file_path, "特色一實施計畫")."<br>";
									if($a5_edu_p2_money > 0) echo get_url($a5_2, $file_path, "特色二實施計畫")."<br>";  // 特色二有錢才顯示。105.01.08新增
								}
							?>
						</td>
					</tr>

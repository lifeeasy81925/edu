<?php
	...
	//$file_path = '/epa_uploads/'.$school_year.'/pub/'.$account.'/';     //j10407，修改路徑
...
<p><b>補助項目三：修繕離島或偏遠地區師生宿舍</b>　<a href="/edu/modules/xSchoolForm/<?=$school_year;?>/allowance-03.htm" target="_blank">(補助項目說明)</a><br />
說明：本項目最高補助140萬元。
</p>
<form name="form" method="post" action="examine_allowance_a3_finish.php" onSubmit="return toCheck();" onKeyDown="if(event.keyCode == 13) return false;">
...
<table border="0">
	...
	<tr>
		<td nowrap="nowrap" >
			<?=($school_year-1);?>年度 <a href="../<?=($school_year-1);?>/effect_view_school_up_03.php?school=<?=$account;?>" target="_blank">執行計畫與經費</a>
		</td>
	</tr>
	<tr>
		<td nowrap="nowrap" >
			<?=($school_year-2);?>年度 <a href="../effect_102_view_03.php?school=<?=$account;?>" target="_blank">執行成果</a>
		</td>
	</tr>
</table>
<p>
...
<font color=blue>※補助師生宿舍-初審金額：<input type="text" size="6" name="city_total_money" value="<?=$city_total_money;?>" style="border:0px; text-align: right;" readonly >
元 (本列自動計算)</font><p>
<? if ($apply_name_t == '') { echo "<!--"; } ?>
<table border="0">
	<tr>
	...
	<tr>
		<td nowrap="nowrap" colspan="2">
			...
			其中包含經常門經費<input type="text" size="2" name="city_p1_current_cnt" value="<?=$city_p1_current_cnt;?>" style="border:0px; text-align: right;" readonly >項
			<input type="text" size="6" name="city_p1_current_money" value="<?=$city_p1_current_money;?>" style="border:0px; text-align: right;" readonly >元，
			資本門經費<input type="text" size="2" name="city_p1_capital_cnt" value="<?=$city_p1_capital_cnt;?>" style="border:0px; text-align: right;" readonly >式
			<input type="text" size="6" name="city_p1_capital_money" value="<?=$city_p1_capital_money;?>" style="border:0px; text-align: right;" readonly >元。
			</b>
		</td>
	</tr>
</table>
...
<table border="0">
	...
	<tr>
		<td nowrap="nowrap" colspan="2">
			...
			其中包含經常門經費<input type="text" size="2" name="city_p2_current_cnt" value="<?=$city_p2_current_cnt;?>" style="border:0px; text-align: right;" readonly >項
			<input type="text" size="6" name="city_p2_current_money" value="<?=$city_p2_current_money;?>" style="border:0px; text-align: right;" readonly >元，
			資本門經費<input type="text" size="2" name="city_p2_capital_cnt" value="<?=$city_p2_capital_cnt;?>" style="border:0px; text-align: right;" readonly >式
			<input type="text" size="6" name="city_p2_capital_money" value="<?=$city_p2_capital_money;?>" style="border:0px; text-align: right;" readonly >元。
			</b>
		</td>
	</tr>
</table>
...
<table border="0" style="width:750px;table-layout:fixed;word-break:break-all">
	...
	<tr>
		...
		<td nowrap="nowrap">
			...
			近三年住宿人員資料：<? echo ($a3_2 == ''?"<font color=red>未上傳</font>":"<font color=blue>已上傳</font>，檔名：<a href='".$file_path2.$a3_2."' target='_new'>$a3_2</a>");?><br />
...
<script language="JavaScript">
	...
	{
		//加總檢核，不得大於140萬元
		if (document.form.city_total_money.value > 1400000) 
		{
			alert('申請總額錯誤！\n申請總額不得大於140萬元。');
			return false;
		}
	...
	function count_all()
	{
		...
		for(i = 1;i < 10;i++)
		{
			...
			var city_p_current_cnt = document.getElementsByName('city_p' + i + '_current_cnt')[0]; //ex.city_p1_current_cnt
			var city_p_current_money = document.getElementsByName('city_p' + i + '_current_money')[0]; //ex.city_p1_current_money 
			var city_p_capital_cnt = document.getElementsByName('city_p' + i + '_capital_cnt')[0]; //ex.city_p1_capital_cnt
			...
			if(city_p_money != null)
			{
				...
				city_p_current_cnt.value = current_cnt;
				city_p_current_money.value = current_money; //各特色的經常門金額
				city_p_capital_cnt.value = capital_cnt;

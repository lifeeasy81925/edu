...
<body onload="set_default()">
	...
	<form name="form" method="post" action="examine_allowance_a4_finish.php" onSubmit="return toCheck();">
			...
			function toCheck()
			{
				...
				for(i = 1; i <= 2; i++)  // 特色(p1、p2)
				{
					for(j = 1; j <= 20; j++)  // 項目
					{
						...
						{
							if(document.getElementsByName('p' + i + '_' + k + '_s_money_'    + j)[0] != null && document.getElementsByName('p' + i + '_' + k + '_city_money_' + j)[0] != null)  // 先檢查呼叫物件，有存在才繼續，不然程式會功能失效。
							{

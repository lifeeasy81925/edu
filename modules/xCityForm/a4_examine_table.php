...
while($row = mysql_fetch_row($result_element)){
	$total = $row[7]+$row[8]+$row[30]+$row[31];
	$exam_money_1 = $row[41];
	$exam_money_2 = $row[42];
	$exam_money_3 = $row[64];
	$exam_money_4 = $row[65];
	$total_city = $exam_money_1 + $exam_money_2 + $exam_money_3 + $exam_money_4;
	$pass = $row[69];
	if($total > '0'){
		$serial++;
		echo "<tr>";
		...
		if($total_city == '0' && $pass == '0'){
		 	echo "<a href="."examine_allowance4.php?id=$row[0]"." target="."_self".">"."待審核"."</a>";
			echo "("."NT$".number_format($total_city).")";
		}elseif($total_city >= '0' && $pass == '1'){
			echo "<a href="."examine_allowance4.php?id=$row[0]"." target="."_self".">"."已審畢"."</a>";
			echo '<img src='.'"../../images/ok1.jpg"'.' width="18"'.' height="18"'.'>';
			echo "("."NT$".number_format($total_city).")";
		}else{
			echo "<a href="."examine_allowance4.php?id=$row[0]"." target="."_self".">"."退件"."</a>";
			echo '<img src='.'"../../images/ok1.jpg"'.' width="18"'.' height="18"'.'>';
			echo "("."NT$".number_format($total_city).")";
		}
		echo "</td>";

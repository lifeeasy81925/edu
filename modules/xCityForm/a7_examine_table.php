...
while($row = mysql_fetch_row($result_element)){
	...
	$exam_money_2 = $row[46];
	$exam_money_3 = $row[47];
	$total_city = $exam_money_1 + $exam_money_2 + $exam_money_3 ;
	$pass = $row[72];
	if($total > '0'){
		$serial++;
		echo "<tr>";
		...
		if($total_city == '0' && $pass == '0'){
			echo "<a href="."examine_allowance7.php?id=$row[0]"." target="."_self".">"."待審核"."</a>";
			echo "("."NT$".number_format($total_city).")";
		}elseif($total_city >= '0' && $pass == '1'){
			echo "<a href="."examine_allowance7.php?id=$row[0]"." target="."_self".">"."已審畢"."</a>";
			echo '<img src='.'"../../images/ok1.jpg"'.' width="18"'.' height="18"'.'>';
			echo "("."NT$".number_format($total_city).")";
		}else{
			echo "<a href="."examine_allowance7.php?id=$row[0]"." target="."_self".">"."退件"."</a>";
			echo '<img src='.'"../../images/ok1.jpg"'.' width="18"'.' height="18"'.'>';
			echo "("."NT$".number_format($total_city).")";
		}
		echo "</td>";

...
<table width="100%" border="1" cellspacing="0" cellpadding="0">
  <tr>
    <td height="60" colspan="9" align="center"><strong><font color=blue>教育部100年度推動教育優先區計畫直轄市、縣(市)政府各補助項目執行成果一覽表<br />縣市別：<? echo $city;?>　　　　　　　　補助項目：五、充實學校基本教學設備</font></strong></td>
  </tr>
  <tr>
...
while($row = mysql_fetch_row($result_school)){
	...
	//補五
	if($row[71] <> ''){$ef_1 = $row[71];}else{$ef_1 = 0;} //該校填報欄位1
	if($row[72] <> ''){$ef_2 = $row[72];}else{$ef_2 = 0;}
	if($row[73] <> ''){$ef_3 = $row[73];}else{$ef_3 = 0;}
	if($row[74] <> ''){$ef_4 = $row[74];}else{$ef_4 = 0;}
	if($row[75] <> ''){$ef_5 = $row[75];}else{$ef_5 = 0;}
	if($row[76] <> ''){$ef_6 = $row[76];}else{$ef_6 = 0;}
	if($row[77] <> ''){$ef_7 = $row[77];}else{$ef_7 = 0;}
	if($row[78] <> ''){$ef_8 = $row[78];}else{$ef_8 = 0;}
	if($row[79] <> ''){$ef_9 = $row[79];}else{$ef_9 = 0;}
	if($row[80] <> ''){$ef_10 = $row[80];}else{$ef_10 = 0;}
	if($row[81] <> ''){$ef_11 = $row[81];}else{$ef_11 = 0;}
	if($row[82] <> ''){$ef_12 = $row[82];}else{$ef_12 = 0;}
	if($row[83] <> ''){$ef_13 = $row[83];}else{$ef_13 = 0;}
	$ef_14 = $row[84];
	$ef_15 = $row[85];
...
?>
  ...
  <tr>
    <td height="40" colspan="9">承辦人：　　　　　　　　　　　　　　　　　　　　主管科長：　　　　　　　　　　　　　　　　　　　　局 (處) 長：</td>
  </tr>
</table>
<table>
</table>

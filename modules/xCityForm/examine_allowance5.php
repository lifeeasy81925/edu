...
//其他資料	
if($class == '1' ){
	$basedata="100element_allowance5";
}else{
	$basedata="100junior_allowance5";
}			
...
if($examine == '2'){
	...
	$result = mysql_query($sql);
	while($row = mysql_fetch_row($result)){
		$city_examine_1 = $row[9];//縣市審核金額合計
		$city_examine_1_1 = $row[40];//縣市審核經常門
		$city_examine_1_2 = $row[41];//縣市審核資本門
	}	
}		
?> 
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<p>補助項目五：充實學校基本教學設備</p>
<form name="form" method="post" action="examine_allowance5_finish.php?id=<? echo $id;?>" onKeyDown="if(event.keyCode == 13) return false;">
...
?>
※申請資料與審核：<br>
●申請經費<font face="Arial, Helvetica, sans-serif" color="#FF0000" size="+1"><? echo number_format($a);?></font>。經常門<font face="Arial, Helvetica, sans-serif" color="#FF0000" size="+1"><? echo number_format($a1);?></font>元，資本門<font face="Arial, Helvetica, sans-serif" color="#FF0000" size="+1"><? echo number_format($a2);?></font>元。<p>
<? if($examine == '2') echo '●縣市審核'.'<font face="Arial, Helvetica, sans-serif" color="#FF0000" size="+1">'.number_format($city1).'</font>元(經常門<font face="Arial, Helvetica, sans-serif" color="#FF0000" size="+1">'.number_format($city1_1).'</font>元，資本門<font face="Arial, Helvetica, sans-serif" color="#FF0000" size="+1">'.number_format($city1_2).'</font>元)'?>
...
if($examine == '2' ){
	if($edu1_1 != 0){
		if($city_examine_1_1 != $edu1_1){
			echo $edu1_1;
		}else{
			echo $city_examine_1_1;
		}
	}else{
		echo $edu1_1;
	}
...
if($examine == '2' ){
	if($edu1_2 != 0){
		if($city_examine_1_2 != $edu1_2){
			echo $edu1_1;
		}else{
			echo $city_examine_1_2;
		}
	}else{
		echo $edu1_2;
	}
...
<!--審查退件功能  -->
	...
	<input type="hidden" name="allowance5"  value="<? echo "allowance5"; ?>">
<label>
	<input type="radio" name="allowance5_pass"  value="1" id="1" <? if ($pass == '1') echo 'checked';?>/>
    審核通過</label>
<label>
    <input type="radio" name="allowance5_pass"  value="2" id="2" <? if ($pass == '2') echo 'checked';?>/>
    審核不通過且列入退件名單</label>
<br><br>

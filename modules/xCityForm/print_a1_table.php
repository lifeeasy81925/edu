...
補助項目一,親職教育<br>
<? /*
	$sql_element_allowance = "SELECT * FROM 100element_allowance where school like '%$city%' and pointer1=1;";//符合有補助項目1的國小
    $result_element_allowance= mysql_query($sql_element_allowance);
	while($row = mysql_fetch_row($result_element_allowance)){
		$p1 = $row[1]; 
	}
	$sql_junior_allowance = "SELECT * FROM 100junior_allowance where school like '%$city%' and pointer1=1;";//符合有補助項目1的國中
    $result_junior_allowance = mysql_query($sql_junior_allowance);
	while($row = mysql_fetch_row($result_junior_allowance)){
		$p2 = $row[1];
	}*/
	$sql_element = "SELECT * FROM 100element_money where school like '%$city%' and pointer1=1 and a1_1='0' and a1_2='0';";//國小資料
...
<table width="500" border="1">
    ...
    <td>申請條件</td>
<?	
	$sql_junior = "SELECT * FROM 100junior_money where school like '%$city%' and pointer1=1 and a1_1='0' and a1_2='0';";

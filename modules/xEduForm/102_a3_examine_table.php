...
<INPUT TYPE="button" VALUE="回前頁" onClick="location='102_city_list_1.php?id=3'">
<?
...
<table width="500" border="1">
    ...
    <td>學校名稱</td>
    <td>縣市初審</td>
    <td align="center">複審結果<br />(按<font color=red><b>F5</b></font>可<font color=red><b>重新整理</b></font>結果)</td>
<?
//合併二資料表再籂選
...
$sql = "select * from 102allowance3 where cityname like '%$cityname%' order by account;";
...
while($row = mysql_fetch_row($result)){
	...
	$pass_city = $row[130];		//縣市審核狀態
	$total_city = $row[132];	//縣市審核金額
	$total_city_1 = $row[133];	//縣市審核經常門
	$total_city_2 = $row[134];	//縣市審核資本門

	$pass = $row[190];			//MOE審核狀態
	$total_edu = $row[192];		//MOE審核金額
	$exam_money_1 = $row[193];	//MOE審核經常門
	$exam_money_2 = $row[194];	//MOE審核資本門

	//$allowance_1 = $row[18];	//符合補助項目1申請資格
	//是否符合指標 且 申請金額大於0
	//echo "<br>".$total;
	//if($total_city > 0 && ($p152>0 || $p153>0) && ($p160>0 || $p161>0)){
	if($total_city > 0 && $pass_city == 1 && ($p152>0 || $p153>0) && ($p160>0 || $p161>0)){
		$serial++;
		echo "<tr>";
		...
		echo "</td>";
		echo "<td>";
		echo "NT$".number_format($total_city);	//預算總合		
		echo "</td>";
		echo "<td>";
		if($total_edu == 0 && $pass == 0){ //若審核金額==0 且 審核狀態==0
		 	echo "<a href="."102_examine_allowance3.php?id=$row[0]"." target="."_blank".">"."<font color=blue>待審核</font>"."</a>";
			echo "("."NT$".number_format($total_edu).")";
		}elseif($total_edu >= 0 && $pass == 1){ //若審核金額>=0 且 審核狀態==1
			echo "<a href="."102_examine_allowance3.php?id=$row[0]"." target="."_blank".">"."已審畢"."</a>";
			echo '<img src='.'"../../images/ok1.jpg"'.' width="18"'.' height="18"'.'>';//顯示已審畢圖示
			echo "("."NT$".number_format($total_edu).")";
		}else{ //非待審核 且 非已審畢 作為不通過處理
			//echo '<img src='.'"../../images/ok2.jpg"'.' width="20"'.' height="20"'.'>';
			echo "<a href="."102_examine_allowance3.php?id=$row[0]"." target="."_blank".">"."<font color=red>不通過</font>"."</a>";

...
<INPUT TYPE="button" VALUE="回前頁" onClick="location='102_city_list_1.php?id=4'">
<?
...
$sql = "select * from 102allowance4 where cityname like '%$cityname%' order by account;";
...
while($row = mysql_fetch_row($result)){
	...
	//是否符合指標 且 申請金額大於0
	//echo "<br>".$total;
	//if($total_city > 0 && $p154 > 0 && $p162 > 0){
	if($total_city > 0 && $pass_city == 1 && $p154 > 0 && $p162 > 0){
		$serial++;
		echo "<tr>";
		...
		if($total_edu == 0 && $pass == 0){ //若審核金額==0 且 審核狀態==0
		 	echo "<a href="."102_examine_allowance4.php?id=$row[0]"." target="."_blank".">"."<font color=blue>待審核</font>"."</a>";
			echo "("."NT$".number_format($total_edu).")";
		}elseif($total_edu >= 0 && $pass == 1){ //若審核金額>=0 且 審核狀態==1
			echo "<a href="."102_examine_allowance4.php?id=$row[0]"." target="."_blank".">"."已審畢"."</a>";
			...
			echo "<a href="."102_examine_allowance4.php?id=$row[0]"." target="."_blank".">"."<font color=red>不通過</font>"."</a>";

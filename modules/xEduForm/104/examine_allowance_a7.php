<?php
	...
	function print_item($allowance_table_name, $p, $main_seq)
	{
		...
		if($has_outlay == 1) //有雜支才顯示
		{
			...
			echo "<td align='left'><input type='text' size='6' name='".$p."_edu_delete_$idx' value='$outlay_edu_delete' onChange='js:Count(this,$idx);'></td>";
			echo "</tr>";
...
<p><b>補助項目七：整修學校社區化活動場所</b>　<a href="/edu/modules/xSchoolForm/<?=$school_year;?>/allowance-07.htm" target="_blank">(補助項目說明)</a><br />
說明：限申請補助綜合球場，補助上限：修建40萬元，設備20萬元。
</p>
<form name="form" method="post" action="examine_allowance_a7_finish.php?a=<?=$a;?>" onSubmit="return toCheck();" onKeyDown="if(event.keyCode == 13) return add_crlf();">
...
<table border="0">
	...
	<tr>
		<td nowrap="nowrap" >
			<?=($school_year-1);?>年度 <a href="../103/effect_view_school_up_07.php?school=<?=$account;?>" target="_blank">執行計畫與經費</a>
		</td>
	</tr>
	<tr>
		<td nowrap="nowrap" >
			<?=($school_year-2);?>年度 <a href="../effect_102_view_07.php?school=<?=$account;?>" target="_blank">執行成果</a>
		</td>
	</tr>
</table>
<p>
...
※補助整修學校社區化活動場所-初審金額： <?=$city_total_money;?> 元<br />
<font color=blue>※補助整修學校社區化活動場所-複審金額：<input type="text" size="6" name="edu_total_money" value="<?=$edu_total_money;?>" style="border:0px; text-align: right;" readonly >
...
<table border="0">
	...
	<tr>
		...
		其中包含修建 <?=$city_p1_current_money;?> 元，
		設備 <?=$city_p1_capital_money;?> 元。
		</td>
	</tr>
	<tr>
		<td nowrap="nowrap" colspan="2">
			...
			其中包含修建<input type="text" size="6" name="edu_p1_current_money" value="<?=$edu_p1_current_money;?>" style="border:0px; text-align: right;" readonly >元，
			設備<input type="text" size="6" name="edu_p1_capital_money" value="<?=$edu_p1_capital_money;?>" style="border:0px; text-align: right;" readonly >元。
			</b>
		</td>
	</tr>
</table>

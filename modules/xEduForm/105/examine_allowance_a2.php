<?php
	...
	function print_item($allowance_table_name, $p, $main_seq)
	{
		...
		while($row = mysql_fetch_array($result))
		{
			...
			$edu_delete = ($row['edu_delete'] == '')?0:$row['edu_delete'];    //原本的
			
			//j-10407,先重算一次$edu_delete
			
			//$edu_delete = $city_money - $edu_money;
	...
	$sql2 = " select sun_ly.point2 as point2_ly, sun_ly.lastyear_leaving_file as lastyear_leaving_file_ly, sun_ly.a2_1 as a2_1_ly, sun_ly.a2_2 as a2_2_ly  ".
	...
	while($row = mysql_fetch_array($result2))
	{
		...
		$lastyear_leaving_file_ly = $row['lastyear_leaving_file_ly'];
		$a2_1_ly = $row['a2_1_ly'];
		$a2_2_ly = $row['a2_2_ly'];
	}
...
<form name="form" method="post" action="examine_allowance_a2_finish.php?a=<?=$a;?>" onSubmit="return toCheck();" onKeyDown="if(event.keyCode == 13) return add_crlf();">
...
<table border="0">
	...
	<tr>
		<td nowrap="nowrap" >
			<?=($school_year-1);?>年度 <a href="../104/effect_view_school_up_02.php?id=<?=$account;?>" target="_blank">執行計畫與經費</a>
		</td>
	</tr>
	<!--
	<tr>
...
<table border="0">
	...
	<tr>
		<td nowrap="nowrap" >
			※補助學校發展特色-初審金額：<?=$school_year;?>年度：<input type="text" size="6" name="city_total_money" value="<?=$city_total_money;?>" style="border:0px; text-align: right;" readonly >元
		</td>
	</tr>
	<tr style="display:<?=($s_total_money_ny1 == 0 || $s_total_money_ny1 == '')?"none":"";?>;">
		<td nowrap="nowrap" >
			※補助學校發展特色-初審金額：<?=($school_year+1);?>年度：<input type="text" size="6" name="city_total_money_ny1" value="<?=$city_total_money_ny1;?>" style="border:0px; text-align: right;" readonly >元
		</td>
	</tr>
	<tr style="display:<?=($s_total_money_ny2 == 0 || $s_total_money_ny2 == '')?"none":"";?>;">
		<td nowrap="nowrap" >
			※補助學校發展特色-初審金額：<?=($school_year+2);?>年度：<input type="text" size="6" name="city_total_money_ny2" value="<?=$city_total_money_ny2;?>" style="border:0px; text-align: right;" readonly >元
		</td>
	</tr>
	<tr>
		<td nowrap="nowrap" >
			<font color=blue>※補助學校發展特色-複審金額：<?=$school_year;?>年度：<input type="text" size="6" name="edu_total_money" value="<?=$edu_total_money;?>" style="border:0px; text-align: right;" readonly >元 (本列自動計算)</font>
			百分比<input type="text" size="6" name="persent" value="<?=$persent;?>" style="border:0px; text-align: right;" readonly >%
		</td>
	</tr>
	<tr style="display:<?=($s_total_money_ny1 == 0 || $s_total_money_ny1 == '')?"none":"";?>;">
		<td nowrap="nowrap" >
			<font color=blue>※補助學校發展特色-複審金額：<?=($school_year+1);?>年度：<input type="text" size="6" name="edu_total_money_ny1" value="<?=$edu_total_money_ny1;?>" style="border:0px; text-align: right;" readonly >元 (本列自動計算)</font>
			百分比<input type="text" size="6" name="persent_ny1" value="<?=$persent_ny1;?>" style="border:0px; text-align: right;" readonly >%
		</td>
	</tr>
	<tr style="display:<?=($s_total_money_ny2 == 0 || $s_total_money_ny2 == '')?"none":"";?>;">
		<td nowrap="nowrap" >
			<font color=blue>※補助學校發展特色-複審金額：<?=($school_year+2);?>年度：<input type="text" size="6" name="edu_total_money_ny2" value="<?=$edu_total_money_ny2;?>" style="border:0px; text-align: right;" readonly >元 (本列自動計算)</font>
	...
	<tr>
		...
		，目標學生佔參加學生 <?=$s_p1_per;?>%。
			<?/*
				if($p1_IsContinue == "Y")
				{					
					echo " <br />●<a href='103_examine_allowance_a2.php?id=$account' target='_blank'>查看".($school_year-1)."年度資料</a>";
					
					if($p1_ContinueYear > 1)
						echo " ●<a href='102_examine_allowance2.php?id=$account' target='_blank'>查看".($school_year-2)."年度資料</a>";
...
<?
	//顯示	細項
	print_item($table_name_item, 'p1_'.($school_year+1), $main_seq);
...
<?
	//顯示	細項
	print_item($table_name_item, 'p1_'.($school_year+2), $main_seq);
...
<?
	//顯示	細項
	print_item($table_name_item, 'p2_'.($school_year+1), $main_seq);
...
<?
	//顯示	細項
	print_item($table_name_item, 'p2_'.($school_year+2), $main_seq);
...
<? if($inherit_year == '104') echo "--!>"; ?>
<? if($inherit_year == '' || $inherit_year == '104_change') echo "<!--"; ?>
	<tr>
		<td nowrap="nowrap" style="width:120px; text-align:right; vertical-align:top;"><b>※104應上傳檔案：</br>
		</td>
		<td nowrap="nowrap">
			</br>特色一實施計畫：<? echo ($a2_1_ly == ''?"<font color=red>未上傳</font>":"<font color=blue>已上傳</font>，檔名：<a href='".$file_path3.$a2_1_ly."' target='_new'>$a2_1_ly</a>");?><br />
			<div style="display:<?=($a2_2_ly == '')?'none':''; //空白就不顯示?>;">
			特色二實施計畫：<? echo ($a2_2_ly == ''?"<font color=red>未上傳</font>":"<font color=blue>已上傳</font>，檔名：<a href='".$file_path3.$a2_2_ly."' target='_new'>$a2_2_ly</a>");?><br />
			</div>
			<div style="display:<?=($page3_warning == '')?'none':''; //空白就不顯示?>;">
			目標學生名冊：<? echo ($point2_ly == ''?"<font color=red>未上傳</font>":"<font color=blue>已上傳</font>，檔名：<a href='".$file_path4.$point2_ly."' target='_new'>$point2_ly</a>");?><br />
			</div>
			<p>
		</td>
	</tr>
...
<script language="JavaScript">
	...
	function Count(obj, item_idx) 
	{
		...
		if(flag == 1)
			edu_money.value = parseInt(city_money.value) - parseInt(edu_delete.value);  //原本
		
			//edu_delete.value = parseInt(edu_money.value) - parseInt(city_money.value);
	...
	function add_crlf()
	{
		...
		var obj_text1 = document.getElementsByName('edu_desc_ny1')[0];
		if(document.activeElement == obj_text || obj_text1)
		{
			return true;
		}
		else

...
<table border="0">
	...
	<tr>
		<td nowrap="nowrap" >
			102年度 <a href="../effect_102_view_up_05.php?school=<?=$account;?>" target="_blank">執行計畫與經費</a>
		</td>
	</tr>
	<tr>
		<td nowrap="nowrap" >
			101年度 <a href="../effect_101_view_05.php?school=<?=$account;?>" target="_blank">執行成果</a>
		</td>
	</tr>
</table>
<p>
...
●申請 <?=$p_num;?> 項特色
<p>
※補助發展原住民文化特色及設備器材-申請補助經費： <?=$s_total_money;?> 元<br />
※補助發展原住民文化特色及設備器材-初審金額： <?=$city_total_money;?> 元<br />
<font color=blue>※補助發展原住民文化特色及設備器材-複審金額：<input type="text" size="6" name="edu_total_money" value="<?=$edu_total_money;?>" style="border:0px; text-align: right;" readonly >
	...
	<tr>
		<td nowrap="nowrap" colspan="2"><b>※特色一：</b><?=$p1_name;?> &nbsp; <?=($p1_IsContinue == "Y")?"(本年度為第 ".$p1_ContinueYear." 年辦理)":"";?>
			<?
				if($p2_IsContinue == "Y")
				{					
					echo " ●<a href='102_examine_allowance5.php?id=$account' target='_blank'>查看102年度資料</a>";
					
					if($p2_ContinueYear > 1)
						echo " ●<a href='101_examine_allowance6.php?id=$account' target='_blank'>查看101年度資料</a>";
				}
			?>
		</td>
	</tr>
...
<table border="0" style="width:750px;table-layout:fixed;word-break:break-all">
	...
	<tr>
		...
		<td nowrap="nowrap">
		實施計畫：<? echo ($a5_1 == ''?"<font color=red>未上傳</font>":"<font color=blue>已上傳</font>，檔名：<a href='".$file_path.$a5_1."' target='_new'>$a5_1</a>");?><br />
...
<script language="JavaScript">
	...
	{		
		...
		for(i = 1;i <= 2;i++)
		{
			var edu_p_money = document.getElementsByName('edu_p' + i + '_money')[0];
			if(edu_p_money != null)
			{
				var p;
				...
				for(j = 1;j <= 10;j++) //計算各特色經常 & 資本，
				{
					...
					if(edu_money != null)
					{
						if(opt_category.value == '加班費' && edu_money.value != '')
							overtime = parseInt(overtime) + parseInt(edu_money.value);
					}
				}
				...
				var diff = parseInt(edu_money.value) - parseInt(overtime);
				if(overtime > 0 && overtime * 10 > diff)
				{
					var x = parseInt(diff * 0.1);
					alert(p + "的加班費不能超過" + p + "申請金額(不含加班費)的10%\n" + p + "申請金額(不含加班費)為 " + diff + "元\n固加班費不能超過 " + x + "元\n");
						return false;
				}
			}
		}

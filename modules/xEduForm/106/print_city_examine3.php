...
<?
	...
	$sql = " select sd.account, sd.schooltype, sd.cityname, sd.district, sd.schoolname, sd.area, sd.getmoney_85to91, sd.traffic_status ".
		   ...
		   //補五縣市資料
		   "      , a5.s_total_money as a5_s_money, a5.item as a5_item ".
	...
	while($row = mysql_fetch_array($result))
	{
		...
		$a4_s_p2_money = $row['a4_s_p2_money'];
		$a4_s_p2_current_money = $row['a4_s_p2_current_money'];
		$a4_s_p2_capital_money = $row['a4_s_p2_capital_money'];

		$a5_item = $row['a5_item'];
		$a5_s_money = $row['a5_s_money'];

		$a6_s_p1_money = $row['a6_s_p1_money'];
		$a6_s_p1_current_money = $row['a6_s_p1_current_money'];
		$a6_s_p1_capital_money = $row['a6_s_p1_capital_money'];
		...
		for($i = 0;$i < 22;$i++) //縣市共22個
		{
			if($apply_money > 0 && $cityname == $city_ary[$i])
			{
				...
				$a3_s_p1_current_money_ary[$i] += $a3_s_p1_current_money;//補三經常
				$a3_s_p1_capital_money_ary[$i] += $a3_s_p1_capital_money;//補三資本
				$a3_total_money_ary[$i] = $a3_s_p1_current_money_ary[$i] + $a3_s_p1_capital_money_ary[$i];

				$a4_current_money_ary[$i] += $a4_s_p1_current_money + $a4_s_p2_current_money;//補四經常
				$a4_capital_money_ary[$i] += $a4_s_p1_capital_money + $a4_s_p2_capital_money;//補四資本
				$a4_total_money_ary[$i] = $a4_current_money_ary[$i] + $a4_capital_money_ary[$i];

				$a5_s_money_1[$i] += ($a5_item == '租車費')?$a5_s_money:0;
				$a5_s_money_2[$i] += ($a5_item == '交通費')?$a5_s_money:0;
				$a5_s_money_3[$i] += ($a5_item == '購置交通車')?$a5_s_money:0;
				$a5_total_money_ary[$i] = $a5_s_money_1[$i] + $a5_s_money_2[$i] + $a5_s_money_3[$i];

				$a6_s_p1_current_money_ary[$i] += $a6_s_p1_current_money;//補六修建
				$a6_s_p1_capital_money_ary[$i] += $a6_s_p1_capital_money;//補六設備
				$a6_total_money_ary[$i] = $a6_s_p1_current_money_ary[$i] + $a6_s_p1_capital_money_ary[$i];
	...
	for($i = 0;$i < 22;$i++) //縣市共22個
	{
		?>
			<tr height="30px">
				...
				<td align="right"><?=number_format($a3_s_p1_current_money_ary[$i]); //補三經常?></td>
				<td align="right"><?=number_format($a3_s_p1_capital_money_ary[$i]); //補三資本?></td>
				...
				<td align="right"><?=number_format($a5_s_money_1[$i]); //補五租車費?></td>
				<td align="right"><?=number_format($a5_s_money_2[$i]); //補五交通費?></td>
				<td align="right"><?=number_format($a5_s_money_3[$i]); //補五購置交通車?></td>
				<td align="right" bgcolor="#FFFFCC"><?=number_format($a5_total_money_ary[$i]);?></td>
				<td align="right"><?=number_format($a6_s_p1_current_money_ary[$i]); //補六修建?></td>
				<td align="right"><?=number_format($a6_s_p1_capital_money_ary[$i]); //補六設備?></td>
				<td align="right" bgcolor="#FFFFCC"><?=number_format($a6_total_money_ary[$i]); ?></td>
				<td align="right" bgcolor="#99CCCC"><?=number_format($s_total_ary[$i]); //各校加總?></td>
			</tr>
		<?
	}
	...
	for($i = 0;$i < 22;$i++) //縣市共22個
	{
		...
		$sum_a3_s_p1_current_money += $a3_s_p1_current_money_ary[$i];//補三經常
		$sum_a3_s_p1_capital_money += $a3_s_p1_capital_money_ary[$i];//補三資本
		$sum_a3_total_money = $sum_a3_s_p1_current_money + $sum_a3_s_p1_capital_money;
		...
		$sum_a5_s_money_1 += $a5_s_money_1[$i];
		$sum_a5_s_money_2 += $a5_s_money_2[$i];
		$sum_a5_s_money_3 += $a5_s_money_3[$i];
		$sum_a5_total_money = $sum_a5_s_money_1 + $sum_a5_s_money_2 + $sum_a5_s_money_3;

		$sum_a6_s_p1_current_money += $a6_s_p1_current_money_ary[$i];//補六修建
		$sum_a6_s_p1_capital_money += $a6_s_p1_capital_money_ary[$i];//補六設備
		$sum_a6_total_money = $sum_a6_s_p1_current_money + $sum_a6_s_p1_capital_money;
...
?>
	...
	<tr height="30px">
		...
		<td align="right"><div align="right"><?=number_format($sum_a5_s_money_1);?></td>
		<td align="right"><div align="right"><?=number_format($sum_a5_s_money_2);?></td>
		<td align="right"><div align="right"><?=number_format($sum_a5_s_money_3);?></td>
		<td align="right" bgcolor="#FFFFCC"><div align="right"><?=number_format($sum_a5_total_money);?></td>
		<td align="right"><div align="right"><?=number_format($sum_a6_s_p1_current_money);?></td>
		<td align="right"><div align="right"><?=number_format($sum_a6_s_p1_capital_money);?></td>
		<td align="right" bgcolor="#FFFFCC"><div align="right"><?=number_format($sum_a6_total_money);?></td>
		<td align="right" bgcolor="#99CCCC"><div align="right"><?=number_format($sum_s_total);?></td>
	</tr>
</table>
</div>
<?

...
<?
	...
	while($row = mysql_fetch_array($result))
	{
		...
		$a4_current_money = $a4_edu_p1_current_money+$a4_edu_p2_current_money;
		$a4_capital_money = $a4_edu_p1_capital_money+$a4_edu_p2_capital_money;
		...
		if($apply_money > 0)
		{
			...
			?>
				<tr height="30px">
					...
					<td align="right"><?=number_format($a3_edu_p1_current_money); //補三經常?></td>
					<td align="right"><?=number_format($a3_edu_p1_capital_money); //補三資本?></td>
					<td align="right" bgcolor="#FFFFCC"><?=number_format($a3_edu_p1_current_money+$a3_edu_p1_capital_money);?></td>
					...
					<td align="right"><?=($a5_item == '租車費')?number_format($a5_edu_money):0; //補五租車費?></td>
					<td align="right"><?=($a5_item == '交通費')?number_format($a5_edu_money):0; //補五交通費?></td>
					<td align="right"><?=($a5_item == '購置交通車')?number_format($a5_edu_money):0; //補五購置交通車?></td>
					<td align="right" bgcolor="#FFFFCC"><?=number_format($a5_edu_money);?></td>
					<td align="right"><?=number_format($a6_edu_p1_current_money); //補六修建?></td>
					<td align="right"><?=number_format($a6_edu_p1_capital_money); //補六設備?></td>
					<td align="right" bgcolor="#FFFFCC"><?=number_format($a6_edu_p1_current_money+$a6_edu_p1_capital_money); ?></td>
			...
			$sum_a3_current += $a3_edu_p1_current_money;
			$sum_a3_capital += $a3_edu_p1_capital_money;
			$sum_a3 += $a3_edu_p1_current_money + $a3_edu_p1_capital_money;
			
			$sum_a4_current += $a4_current_money;
			$sum_a4_capital += $a4_capital_money;
			$sum_a4 += $a4_current_money + $a4_capital_money;
			
			$sum_a5_1 += ($a5_item == '租車費')?$a5_edu_money:0;
			$sum_a5_2 += ($a5_item == '交通費')?$a5_edu_money:0;
			$sum_a5_3 += ($a5_item == '購置交通車')?$a5_edu_money:0;
			$sum_a5 += $a5_edu_money;
			
			$sum_a6_current += $a6_edu_p1_current_money;
			$sum_a6_capital += $a6_edu_p1_capital_money;
			$sum_a6 += $a6_edu_p1_current_money + $a6_edu_p1_capital_money;
			
			$sum_a1_to_a6 += $edu_total;			
		}				
	}
?>
	<tr height="30px">
		...
		<td align="right" bgcolor="#FFCCCC"><div align="right"><?=number_format($sum_a1_to_a6);?></td>
	</tr>
</table>
</div>
<p>

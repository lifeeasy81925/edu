...
【<?=($school_year);?>年度教育部推動教育優先區計畫申請結果縣市列表】
...
<?
	...
	while($row = mysql_fetch_array($result))
	{
		...
		$a6_s_money_1 = ($a6_item == '租車費')?$a6_s_money:0;
		$a6_s_money_2 = ($a6_item == '交通費')?$a6_s_money:0;
		$a6_s_money_3 = ($a6_item == '購置交通車')?$a6_s_money:0;
		...
		for($i = 0;$i < count($city_ary);$i++)
		{
			if($cityname == $city_ary[$i])
			{
				...
				//補六 購交通車費
				//補七 綜合球場	
				$current[$i] += $a1_s_p1_money + $a1_s_p2_money 
							  + $a2_s_p1_current_money + $a2_s_p2_current_money 
							  + $a3_s_p1_current_money + $a3_s_p2_current_money 
							  + $a4_s_p1_current_money 
							  + $a5_s_p1_current_money + $a5_s_p2_current_money
							  + $a6_s_money_1 + $a6_s_money_2;
							  
				$capital[$i] += $a2_s_p1_capital_money + $a2_s_p2_capital_money 
							  + $a3_s_p1_capital_money + $a3_s_p2_capital_money 
							  + $a4_s_p1_capital_money 
							  + $a5_s_p1_capital_money + $a5_s_p2_capital_money
							  + $a6_s_money_3 
							  + $a7_s_p1_current_money + $a7_s_p1_capital_money;
	...
	for($i = 0;$i < count($city_ary);$i++)
	{
		...
		echo "<td nowrap='nowrap' align='right' width='120px' bgcolor='#FFFFCC'>".number_format($s_total_ary[$i])."</td>";
		echo "</tr>";
		
		//合計
		$current_total += $current[$i];
		$capital_total += $capital[$i];
		$s_total += $s_total_ary[$i];

...
<INPUT TYPE="button" VALUE="回前頁" onClick="location='city_list_1.php?id=4'">
<?
...
<? echo $city."_";?>補助項目四：修繕離島或偏遠地區師生宿舍<br /><br />
已申請學校列表：國小
...
while($row = mysql_fetch_row($result_element)){
	$total_city = $row[7]+$row[8]+$row[30]+$row[31]; //縣市審核金額
	$exam_money_1 = $row[51];
	$exam_money_2 = $row[52];
	$exam_money_3 = $row[74];
	$exam_money_4 = $row[75];
	$total_edu = $exam_money_1 + $exam_money_2 + $exam_money_3 + $exam_money_4;	//教育部審核金額
	$pass = $row[79];	//教育部審核狀態
	if($total_city > '0'){
		$serial++;
		echo "<tr>";
		...
		if($total_edu == '0' && $pass == '0'){
		 	echo "<a href="."examine_allowance4.php?id=$row[0]"." target="."_self".">"."待審核"."</a>";//預算審核
			echo "("."NT$".number_format($total_edu).")";
		}elseif($total_edu >= '0' && $pass == '1'){
			echo "<a href="."examine_allowance4.php?id=$row[0]"." target="."_self".">"."已審畢"."</a>";//預算審核
			echo '<img src='.'"../../images/ok2.jpg"'.' width="18"'.' height="18"'.'>';
			echo "("."NT$".number_format($total_edu).")";
		}else{ //非待審核 且 非已審畢 作為退件處理
			echo "<a href="."examine_allowance4.php?id=$row[0]"." target="."_self".">"."退件"."</a>";//預算審核

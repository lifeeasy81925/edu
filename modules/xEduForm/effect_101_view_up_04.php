...
<?
//讀取上傳檔案資料
$sql = "select  *  from   101school_upload_name where account like '$school'";
$result = mysql_query($sql);
while($row = mysql_fetch_row($result)){
	$file_1 = $row[54];
	$file_2 = $row[55];
...
<form action="file_up_101.php" method="post" enctype="multipart/form-data" target="_self">
    ...
    <input type="hidden" name="table" value="e_4_3">
    <input type="file" name="myfile">
<!---    <input type="submit" value="上傳執行成果照片">--->
<br /><? if ($file_3 == ''){echo "檔案狀態：<font color=red>未上傳</font> (上傳後請按F5更新狀態)";} else {echo '檔案狀態：<font color=blue>已上傳</font>，檔名：'.'<a href="../xSchoolForm/upload/101/'.$school.'/'.$file_3.'" target="_new">'.$file_3.'</a>';}?>
  </form>
...
<form action="file_up_101.php" method="post" enctype="multipart/form-data" target="_self">
    <input type="hidden" name="max_file_size" value="102400000">
    <input type="hidden" name="table" value="e_4_1">
    <input type="file" name="myfile">
<!---    <input type="submit" value="上傳核定後執行計畫">--->
<br /><? if ($file_1 == ''){echo "檔案狀態：<font color=red>未上傳</font> (上傳後請按F5更新狀態)";} else {echo '檔案狀態：<font color=blue>已上傳</font>，檔名：'.'<a href="../xSchoolForm/upload/101/'.$school.'/'.$file_1.'" target="_new">'.$file_1.'</a>';}?>
  </form>
<br />
<strong>※上傳核定後執行經費概算表</strong> <font color=red>(必填)</font>：請彙整於Word檔
<form action="file_up_101.php" method="post" enctype="multipart/form-data" target="_self">
    <input type="hidden" name="max_file_size" value="102400000">
    <input type="hidden" name="table" value="e_4_2">
    <input type="file" name="myfile">
<!---    <input type="submit" value="上傳核定後經費概算表">--->
<br /><? if ($file_2 == ''){echo "檔案狀態：<font color=red>未上傳</font> (上傳後請按F5更新狀態)";} else {echo '檔案狀態：<font color=blue>已上傳</font>，檔名：'.'<a href="../xSchoolForm/upload/101/'.$school.'/'.$file_2.'" target="_new">'.$file_2.'</a>';}?>
</form>

...
//第二個經費表
$sql = "select * from 102allowance5_2 where account like '$username' ";
$result = mysql_query($sql);
while($row = mysql_fetch_row($result)){
	$b030 = $row[30]; //經費表1-1
	$b031 = $row[31]; //
...
<form name="form" method="post" action="102school_write_a5_finish.php"  onSubmit="return toCheck();" onKeyDown="if(event.keyCode == 13) return false;">
<p><b>補助項目五：發展原住民教育文化特色及充實設備器材</b>　<a href="/edu/modules/xSchoolForm/download/allowance-05.htm" target="_blank">(補助項目說明)</a></p>
<p>
<font color=blue>※補助發展原住民文化特色及充實設備器材</font><br />
說明：學生100人(含)以下最高補助12萬元，學生超過100人最高補助25萬元，分校分班最高補助8萬元。<br />
※貴校學生人數：<font color=blue><? echo $p100; ?>人</font>
<p>
...
<script language="JavaScript">
  function toCheck() {
	...
	/*
    if ( <? echo $a019;?>==2 && document.form.char2.value == "" ) {
      alert("班級數 <? echo $classes;?>\n請填寫「特色二申請項目名稱」！\n若無請填「無」");
      document.form.char2.focus();
      return false;
    }
    if ( <? echo $a019;?> == 2 && document.form.char2Mon.value == "" ) {
      alert("請填寫「特色二申請經費」！\n若無請填0");
      document.form.char2Mon.focus();
      return false;
    }
    if ( <? echo $a019;?> == 2 && document.form.char2a.value == "" ) {
      alert("請填寫「特色二經常門」！\n若無請填0");
      document.form.char2a.focus();
      return false;
    }
    if ( <? echo $a019;?> == 2 && document.form.char2b.value == "" ) {
      alert("請填寫「特色二資本門」！\n若無請填0");
	...
	*/
	...
	//金額檢核
	if(<? echo $p100;?> < 100 ){var maxMon = 120000; }else{var maxMon = 250000;}
	//alert(maxMon);
    ...
    if ( document.form.char1Mon.value > maxMon) {
		alert('特色一申請金額超過可申請上限！');
		//document.form.char1Mon.focus();
		return false;
    }
    if ( document.form.char2Mon.value > maxMon) {

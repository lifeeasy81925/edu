...
<p><b>補助項目五：發展原住民教育文化特色及充實設備器材</b>　<a href="/edu/modules/xSchoolForm/download/allowance-05.htm" target="_blank">(補助項目說明)</a>
<p>
<font color=blue>※補助發展原住民文化特色及設備器材-申請補助經費：<input type="text" size="6" name="s_total_money" value="<?=$s_total_money;?>" style="border:0px; text-align: right;" readonly >
元 (本列自動計算)</font><br />
說明：學生100人(含)以下最高補助12萬元，學生超過100人最高補助25萬元，分校分班最高補助8萬元，且<font color="red">「須發展原住民的相關特色」</font>；<font color="red">鐘點費(含補充保費)，會自動計入額外2%金額</font>。
<p>
※申請<input type="text" size="6" name="p_num" value="<?=$p_num;?>" >項特色
<p>
<table border="0">
	<tr>
		<td nowrap="nowrap">※特色一：<input type="text" size="20" name="p1_name" value="<?=$p1_name;?>" >(特色名稱)</td>
		<td><input type="checkbox" name="p1_IsContinue" value="Y" <?=($p1_IsContinue == "Y")?"checked":"";?> >本特色為延續性辦理，
			本年度為第<br />　&nbsp;<input type="text" size="2" name="p1_ContinueYear" value="<?=$p1_ContinueYear;?>" >年辦理。</td>
	</tr>
	<tr>
		<td nowrap="nowrap" colspan="2">
			<font color=blue>經費<input type="text" size="6" name="s_p1_money" value="<?=$s_p1_money;?>" style="border:0px; text-align: right;" readonly>元
...
<?
	//顯示特色一 細項
	print_item('alc_aboriginal_education_item', 'p1', $main_seq);
...
<table border="0">
	<tr>
		<td nowrap="nowrap">※特色二：<input type="text" size="20" name="p2_name" value="<?=$p2_name;?>" >(特色名稱)</td>
		<td><input type="checkbox" name="p2_IsContinue" value="Y" <?=($p2_IsContinue == "Y")?"checked":"";?> >本特色為延續性辦理，
			本年度為第<br />　&nbsp;<input type="text" size="2" name="p2_ContinueYear" value="<?=$p2_ContinueYear;?>" >年辦理。</td>
	</tr>
	<tr>
		<td nowrap="nowrap" colspan="2">
...
<?
	//顯示特色二 細項
	print_item('alc_aboriginal_education_item', 'p2', $main_seq);
	...
	function toCheck() 
	{
		...
		for(i = 1;i <= 2;i++)
		{
			...
			if(s_p_money != null)
			{
				...
				var diff = parseInt(s_p_money.value) - parseInt(overtime);
				if(overtime > 0 && overtime * 10 > diff)
				{
					var x = parseInt(diff * 0.1);
					errmsg += p + "的加班費不能超過" + p + "申請金額(不含加班費)的10%\n" + p + "申請金額(不含加班費)為 " + diff + "元\n固加班費不能超過 " + x + "元\n";
					flag = 0;
				}
			}
		}

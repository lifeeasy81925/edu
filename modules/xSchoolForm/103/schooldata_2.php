<?php
	...
	$sql = " select sd.account, sd.schooltype, sd.cityname, sd.district, sd.schoolname, sy.student ".
		   ...
		   "      , sd102.a135, sd102.a138, sd102.a141, sd102.a144 ". //去年編制、調入、實缺、其他
		   "      , sd102.a060, sd102.a137, sd102.a140, sd102.a143 ". //前年編制、調入、實缺、其他
	...
	while($row = mysql_fetch_array($result))
	{
		...
		//若無資料則代入去年的 
		$teacher_change_in_last = ($row['teacher_change_in_last'] == "")?$row['a138']:$row['teacher_change_in_last'];
		$teacher_change_lack_last = ($row['teacher_change_lack_last'] == "")?$row['a141']:$row['teacher_change_lack_last'];
		$teacher_change_other_last = ($row['teacher_change_other_last'] == "")?$row['a144']:$row['teacher_change_other_last'];
		...
		{
			$teacher_last = ($teacher_last == "")?"0":$teacher_last;
			...
			$teacher_last2 = ($teacher_last2 == "")?"0":$teacher_last2;
			$teacher_change_in_last2 = ($teacher_change_in_last2 == "")?"0":$teacher_change_in_last2;
			$teacher_change_lack_last2 = ($teacher_change_lack_last2 == "")?"0":$teacher_change_lack_last2;
			$teacher_change_other_last2 = ($teacher_change_other_last2 == "")?"0":$teacher_change_other_last2;
			
			if($account == '014580-1') //學校有資料但是去年資料庫沒東西
			{
				$teacher_last = 6;
				$teacher_change_in_last = 5;
				$teacher_change_lack_last = 6;
				$teacher_change_other_last = 0;
...
<script language="JavaScript">
	...
	function toCheck() 
	{
		...
		if ( document.form.tnumber1.value >= 40 ) 
		{
			...
			{	
				show_errmsg = 1;
				document.form.page2_warning.value = "「102年全校教師編制人數與去年」比超過±10%，請填寫異動過大說明"
				
				if(document.form.page2_desc.value == "" ) //未填原因才顯示訊息
				{
					errmsg += '「102學年度全校教師編制人數」與「101學年度全校教師編制人數」不得超過±10%'+'﹙102年度全校教師40人以上才檢核﹚\n';
					flag = 0;
				}
			}
		}
		...
		{	
			...
			if(document.form.page2_desc.value == "" ) //未填原因才顯示訊息
			{	
				errmsg += '全校學生數：102學年度全校教師編制人數，比值不可＞20 或 ＜6 ﹙學生數100人以上才檢核﹚\n';
				flag = 0;
			}
		}

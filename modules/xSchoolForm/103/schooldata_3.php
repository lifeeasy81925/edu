<?php
	...
	while($row = mysql_fetch_array($result))
	{
		...
		$aboriginal_last = $row['a113']; //去年原住民
		$target_no_aboriginal_last = $row['a119']; //去年目標學生 不含僅原住民
		$target_aboriginal_last = $row['a120']; //去年目標學生 含原住民
		...
		if($account == '094618-1' || $account == '014580-1' || $account == '134748a1' || $account == '014578' || $account == '014809')
		{
			...
			$aboriginal_last = ($aboriginal_last == "")?"0":$aboriginal_last;
			$target_no_aboriginal_last = ($target_no_aboriginal_last == "")?"0":$target_no_aboriginal_last;
			$target_aboriginal_last = ($target_aboriginal_last == "")?"0":$target_aboriginal_last;
			
			if($account == '014580-1') //學校有資料但是去年資料庫沒東西
			{
				$aboriginal_last = 2;
				$target_no_aboriginal_last = 65;
				$target_aboriginal_last = 67;
			}
...
<p>
<b>一、目標學生數</b>
<input type="button" name="Submit" value="下載目標學生名冊範例檔案" onclick="location.href='../download/目標學生名冊_模擬範例.xls'"> 
<input type="button" name="Submit" value="下載目標學生空白名冊" onclick="location.href='../download/目標學生名冊_空白.xls'">
<p>
...
<table border="0">
	<tr>
		<td nowrap="nowrap">
			去年目標學生數(不含僅具原住民身分者)：<?=$target_no_aboriginal_last?>
		</td>
	</tr>
	<tr>
		<td nowrap="nowrap" >
			去年目標學生數(含僅原住民身分者)：<?=$target_aboriginal_last?>
		</td>
	</tr>
	<tr>
		<td nowrap="nowrap" >
			去年原住民學生數：<?=$aboriginal_last?>
		</td>
	</tr>
</table>
<p>
...
<p>
※本校目標學生數（不含僅具原住民身分者）共：
<input type="text" size="6" name="target_no_aboriginal" value="<?=$target_no_aboriginal;?>"/>人<br />
<font color=blue>　（各類目標學生若同時符合原住民與其他身分，應計入統計，但不含僅具原住民身分學生）</font><br />
※本校目標學生數（含僅原住民身分者）共：
<input type="text" size="6" name="target_aboriginal" value="<?=$target_aboriginal;?>"/>人<br />
<font color=blue>　（各類目標學生總人數，若同一學生具備多種身分，僅算一人）</font>
<p>
<b>二、學習弱勢及中途輟學學生</b>
<p>
※去(<?=$school_year-2;?>)學年度中途輟學學生數：<input type="text" size="6" name="lastyear_leaving" value="<?=$lastyear_leaving;?>"/>人
<input type="button" name="Submit" value="下載中途輟學學生名冊範例檔案" onclick="location.href='/edu_dl/103/中途輟學學生名冊範例.xls'"> <br />
<font color=blue>　（從<?=$school_year-2;?>年9月1日至<?=$school_year-1;?>年6月30日之中輟學生數）</font>
<p>
<? if ($schooltype == "國小") echo "<!--"; ?>
※去(<?=$school_year-2;?>)學年度全校學生 <font color=blue><?=$laststudent;?></font> 人，參加學生基本學力測驗學生數：<input type="text" size="6" name="lastyear_test" value="<?=$lastyear_test;?>"/>人，
，成績PR值≦25學生數：<input type="text" size="6" name="lastyear_pr25" value="<?=$lastyear_pr25;?>"/>人<br />
<font color=blue>　（因多元入學方案取得高中職入學資格，而未參加學生基本學力測驗學生除外）</font>
<? if ($schooltype == "國小") echo "-->"; ?>
<!--檢核警示訊息-->
...
<script language="JavaScript">
	...
	function toCheck() 
	{
		...
		{
			show_errmsg = 1;
			document.form.page3_warning.value = "「目標學生數(不含僅原住民)欄與去年」比超過±20%，請填寫異動過大說明";
						
			if	(document.form.page3_desc.value == "" )
			{   
				errmsg += '「目標學生數(不含僅原住民)」欄與去年('+lastabcd+'人)比較超過±20%\n';
				flag = 0;
			}
		...
		{
			show_errmsg = 1;
			document.form.page3_warning.value = "「目標學生數(含原住民)欄與去年」比超過±20%，請填寫異動過大說明";
		...
		if ( document.form.target_no_aboriginal.value*1 > document.form.target_aboriginal.value*1 ) {
			errmsg += '「目標學生數(不含僅原住民)」不得大於「目標學生數(含原住民)」！\n';
			flag = 0;
		}

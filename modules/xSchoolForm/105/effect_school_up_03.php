...
<strong>項目：三、修繕離島或偏遠地區師生宿舍  </strong><a href="/edu/modules/xSchoolForm/105/allowance-03.htm" target="_blank">（補助項目說明）</a>
<?
	...
	<tr>
		<td>
			<form onSubmit="return check()" action="file_up.php?sy_seq=<?=$sy_seq;?>&school_year=<?=$school_year;?>" method="post" enctype="multipart/form-data" target="_self">
				...
				<input type="hidden" name="column_name" value="effect_a3_1">
	...
	<tr>
		<td>
			<form onSubmit="return check()" action="file_up.php?sy_seq=<?=$sy_seq;?>&school_year=<?=$school_year;?>" method="post" enctype="multipart/form-data" target="_self">
				...
				<input type="hidden" name="max_file_size" value="102400000">
				<input type="hidden" name="column_name" value="effect_a3_2">
				<input type="file" name="myfile">
	...
	<tr>
		<td>
			<form onSubmit="return check()" action="file_up.php?sy_seq=<?=$sy_seq;?>&school_year=<?=$school_year;?>" method="post" enctype="multipart/form-data" target="_self">
				...
				<input type="hidden" name="max_file_size" value="102400000">
				<input type="hidden" name="column_name" value="effect_a3_3">
				<input type="file" name="myfile">
...
<script type="text/javascript">
    $('form').submit(function(e) {
        ...
        e.preventDefault();
		 bootbox.dialog({
                message: "● 為避免個人資料(個資)之外洩及保障個人隱私權，<font color='red'><b>請勿顯示全名、身份證字號、電話、E-mail等個資。詳細資料請自行留存學校備查!!</b></font>",
                title: " 重要提醒!!",
                buttons: {
                      OK: {
                        label: "我已確實了解!!",

...
<strong>項目：四、充實學校基本教學設備  </strong><a href="/edu/modules/xSchoolForm/105/allowance-04.htm" target="_blank">（補助項目說明）</a>
<?
	...
	//讀取上傳檔案資料
	$sql = " select effect_a4_1, effect_a4_2, effect_a4_3, effect_a4_4 ".
	...
	while($row = mysql_fetch_array($result))
	{
		$file_1 = $row['effect_a4_1'];
		$file_2 = $row['effect_a4_2'];
		$file_3 = $row['effect_a4_3'];
		$file_4 = $row['effect_a4_4'];
	}
	...
	<tr>
		<td>
			<form onSubmit="return check()" action="file_up.php?sy_seq=<?=$sy_seq;?>&school_year=<?=$school_year;?>" method="post" enctype="multipart/form-data" target="_self">
				...
				<input type="hidden" name="column_name" value="effect_a4_1">
	...
	<tr>
		<td>
			<form onSubmit="return check()" action="file_up.php?sy_seq=<?=$sy_seq;?>&school_year=<?=$school_year;?>" method="post" enctype="multipart/form-data" target="_self">
				...
				<input type="hidden" name="max_file_size" value="102400000">
				<input type="hidden" name="column_name" value="effect_a4_2">
				<input type="file" name="myfile">
	...
	<tr>
		<td>
			<form onSubmit="return check()" action="file_up.php?sy_seq=<?=$sy_seq;?>&school_year=<?=$school_year;?>" method="post" enctype="multipart/form-data" target="_self">
				...
				<input type="hidden" name="max_file_size" value="102400000">
				<input type="hidden" name="column_name" value="effect_a4_3">
				<input type="file" name="myfile">

...
$sql = " select sd.account, sd.schooltype, sd.cityname, sd.district, sd.schoolname, sd.area ".
		   ...
		   "      , sy.target_aboriginal, sy.target_no_aboriginal, sy.low_income, sy.grandparenting ". //原住民
		   ...
		   "      , sy.teacher_3years_total, sy.teacher, sy.teacher_change_total, sy.teacher_agent_total ". //老師資料
		   ...
		   "      , sy_ly.student as student_ly ".    //104年學生
		   "      , sy_l2y.student as student_l2y ".  //j10407，新增103年學生		   
		   
		   "	  , sy_ly.teacher as teacher_ly ".  //104年老師
           ...
           "      , a4_l2y.edu_total_money as a4_l2y_money ".    //j10407，新增103年
		   ...
		   "                       left join school_examdata as se on sd.account = se.account and se.school_year = '$school_year' ".                         //105
		   "                       left join school_location as sl on sd.account = sl.account ". 
		   
		   //去年資料的資料表
		   "                       left join schooldata_year as sy_ly on sd.account = sy_ly.account and sy_ly.school_year = '".($school_year - 1)."' ".      //104
		   ...
		   //j10407,新增前(103)年資料的資料表
		   "                       left join schooldata_year as sy_l2y on sd.account = sy_l2y.account and sy_l2y.school_year = '".($school_year - 2)."' ".     //103
	...
	while($row = mysql_fetch_array($result))
	{
		...
		$laststudent = ($row['student_ly'] == "")?0:$row['student_ly']; //去(104)年學生數
		$laststudent_l2y = ($row['student_l2y'] == "")?0:$row['student_l2y']; //前(103)年學生數
		...
		$teacher_last = ($row['teacher_ly'] == "")?"0":$row['teacher_ly']; //去(104)年教師編制
		$teacher_change_in_last = $row['teacher_change_in_ly'];                                      //teacher_change_in_last
		$teacher_change_lack_last = $row['teacher_change_lack_ly'];                                  //teacher_change_lack_last
		$teacher_change_other_last = $row['teacher_change_other_ly'];                                //teacher_change_other_last
		...
		$teacher_l2y = ($row['teacher_l2y'] == "")?"0":$row['teacher_l2y']; //前(103)年教師編制
		$teacher_change_in_l2y = $row['teacher_change_in_l2y'];
		$teacher_change_lack_l2y = $row['teacher_change_lack_l2y'];
		$teacher_change_other_l2y = $row['teacher_change_other_l2y']; 
		
		$teacher_change_l2y = $teacher_change_in_l2y + $teacher_change_lack_l2y; //異動人數 調入+實缺
		$teacher_agent_l2y = $teacher_change_lack_l2y + $teacher_change_other_l2y; //代理人數 實缺+其他
		...
		$a4_l2y_money = ($row['a4_l2y_money'] == '')?0:$row['a4_l2y_money'];   //j10407，新增103年
		
		//補四，前三年補助金額，注意不是申請金額，是複審補助金額
		$money_last3 = ($row['a169'] == '')?0:$row['a169'];              //j10407，修改為102年，原($row['a5'] == '')?0:$row['a5'];
		$money_last2 = $a4_l2y_money ;                                   //j10407，修改為103年
...
<br />
表Ｉ～１ 指標界定調查紀錄表
<INPUT TYPE="button" VALUE="關閉本頁" onclick="javascript:window.close()">
...
<?
	...
	echo "　　承辦人：".$user_from;
	echo "　　E-mail：".$email;
	//echo $accord_point.$can_apply.$applied;
?><br />
<table border="1" cellspacing="0" cellpadding="0" align="left" style="font-family:標楷體; width:950px">
	...
	<tr>
		...
		<td height="30" nowrap="nowrap"><p align="right"><? echo $teacher_l2y; ?>(人)</p></td>
	</tr>
	<tr>
		<td height="30" colspan="2"><p>教師編制人數<?=($school_year-2)?>學年</p></td>
		<td height="30" nowrap="nowrap"><p align="right"><? echo $teacher_last; ?>(人)</p></td>                  <!-- $teacher_last -->
	</tr>
	<tr>
	...
	<tr>
		<td height="30" rowspan="7" nowrap="nowrap"><p align="center">三 </p></td>
		<td height="30" rowspan="7" nowrap="nowrap"><p>指標二：<br />目標學生比例偏高之學校</p></td>
		<td height="30" colspan="2"><p>低收入戶子女學生人數 </p></td>
		<td height="30" nowrap="nowrap"><p align="right"><? echo $low_income; ?>(人)</p></td>
		<td height="30" rowspan="7" align="left" nowrap="nowrap">
		<p align="left">
		<?
	...
	<tr>
		<td height="30" colspan="2"><p>新住民子女學生人數 </p></td>
	...
	-->
	<tr>
		<td height="30" colspan="2"><p>目標學生人數</p></td>
	...
	-->
	<tr>
		<td height="30" colspan="2"><p>目標學生佔全校學生比率 </p></td>
		<td height="30" nowrap="nowrap"><p align="right"><? echo number_format($target_aboriginal/$student*100,2); ?>(％)</p></td>
	</tr>
	<tr>
		<td height="30" rowspan="5" nowrap="nowrap"><p align="center">四 </p></td>
		<td height="30" rowspan="5" nowrap="nowrap"><p>指標三：<br />國中學習弱勢學生比例偏高之學校</p></td>
		<td height="30" colspan="2"><p>國文成績待加強人數佔參加會考人數比例</p></td>
		<td height="30" nowrap="nowrap"><p align="right"><?=number_format($chinese/($student_exam-$chinese_missing)*100,2);?>(%)</p></td>    <!-- $student_exam; -->
		<td height="30" rowspan="5" align="left" nowrap="nowrap">
		<p align="left">
		<?
	...
	<tr>
		<td height="30" colspan="2"><p>英文成績待加強人數佔參加會考人數比例</p></td>
		<td height="30" nowrap="nowrap"><p align="right"><?=number_format($english/($student_exam-$english_missing)*100,2);?>(%)</p></td>    <!-- $count_p3; -->
	</tr>
	<tr>
		<td height="30" colspan="2"><p>數學成績待加強人數佔參加會考人數比例</p></td>
		<td height="30" nowrap="nowrap"><p align="right"><?=number_format($math/($student_exam-$math_missing)*100,2);?>(%)</p></td>
	</tr>
	<tr>
		<td height="30" colspan="2"><p>社會成績待加強人數佔參加會考人數比例</p></td>
		<td height="30" nowrap="nowrap"><p align="right"><?=number_format($social/($student_exam-$social_missing)*100,2);?>(%)</p></td>
	</tr>
	<tr>
		<td height="30" colspan="2"><p>自然成績待加強人數佔參加會考人數比例</p></td>
		<td height="30" nowrap="nowrap"><p align="right"><?=number_format($nature/($student_exam-$nature_missing)*100,2);?>(%)</p></td>
	</tr>
	<tr>
		<td height="30" rowspan="3" nowrap="nowrap"><p align="center">五 </p></td>
		<td height="30" rowspan="3" nowrap="nowrap"><p>指標四：<br />中途輟學率偏高之學校</p></td>
		<td height="30" colspan="2"><p>從<?=($school_year-2);?>年9月1日至<?=($school_year-1);?>年6月30日之中輟學生數</p></td>
	...
	<tr>
		<td height="30" colspan="2"><p>前學年在籍學生人數 </p></td>
	...
	<tr>
		...
		<td height="30" nowrap="nowrap"><p align="right"><?=$teacher_change_l2y?>(人)</p></td>              <!--$teacher_change_last2 -->
	</tr>
	<tr>
		<td height="30" nowrap="nowrap"><p>教師異動人數<?=($school_year-2);?>學年度 </p></td>
		<td height="30" nowrap="nowrap"><p align="right"><?=$teacher_change_last;?>(人)</p></td>             <!-- $teacher_change_last  -->
	</tr>
	<tr>
	...
	<tr>
		...
		<td height="30" nowrap="nowrap"><p align="right"><?=$teacher_change+$teacher_change_last+$teacher_change_l2y;?>(人)</p></td>         <!--?=$teacher_change_total;? -->
	</tr>
	<tr>
	...
	<tr>
		...
		<td height="30" nowrap="nowrap"><p align="right"><?=$teacher_agent_l2y;?>(人)</p></td>        <!--$teacher_agent_last2-->
	</tr>
	<tr>
	...
	<tr>
		...
		<td height="30" nowrap="nowrap"><p align="right"><?=$teacher_agent+$teacher_agent_last+$teacher_agent_l2y;?>(人)</p></td>        <!--=$teacher_agent_total -->
	</tr>
	<tr>

<?php
	...
	$sql = " select sd.account, sd.schooltype, sd.cityname, sd.district, sd.schoolname, sd.area, sd.getmoney_85to91, sd.traffic_status ".
		   "      , sy.* ".
		   //補五資料
		   "      , a5.p1_name, a5.p2_name ".
		   "      , a5.s_p1_student, a5.s_p2_student ".
		   "      , a5.s_p1_target, a5.s_p2_target ".
		   "      , a5.s_total_money ".
		   "      , a5.s_p1_money, a5.s_p2_money ".
		   "      , a5.s_p1_current_cnt, a5.s_p1_current_money ".
		   "      , a5.s_p1_capital_cnt, a5.s_p1_capital_money ".
		   "      , a5.s_p2_current_cnt, a5.s_p2_current_money ".
		   "      , a5.s_p2_capital_cnt, a5.s_p2_capital_money ".
		   "      , a5.p1_IsContinue, a5.p2_IsContinue, a5.p_num ".
		   "      , a5.p1_ContinueYear, a5.p2_ContinueYear ".
		   
		   "      , a5.s_total_money_ny1 ".
		   "      , a5.s_p1_money_ny1, a5.s_p2_money_ny1 ".
		   "      , a5.s_p1_current_cnt_ny1, a5.s_p1_current_money_ny1 ".
		   "      , a5.s_p1_capital_cnt_ny1, a5.s_p1_capital_money_ny1 ".
		   "      , a5.s_p2_current_cnt_ny1, a5.s_p2_current_money_ny1 ".
		   "      , a5.s_p2_capital_cnt_ny1, a5.s_p2_capital_money_ny1 ".
		   
		   "      , a5.s_total_money_ny2 ".
		   "      , a5.s_p1_money_ny2, a5.s_p2_money_ny2 ".
		   "      , a5.s_p1_current_cnt_ny2, a5.s_p1_current_money_ny2 ".
		   "      , a5.s_p1_capital_cnt_ny2, a5.s_p1_capital_money_ny2 ".
		   "      , a5.s_p2_current_cnt_ny2, a5.s_p2_current_money_ny2 ".
		   "      , a5.s_p2_capital_cnt_ny2, a5.s_p2_capital_money_ny2 ".
	...
	function print_item($allowance_table_name, $p, $main_seq)
	{
		...
		//-----	
			...
			$s1 = array("","經常門","資本門");
			$s3 = array(""=>array("")
					 , "經常門"=>array("","鐘點費","鐘點費(含補充保費)","器材購置","器材維護","教材","道具","旅運費","其他")
...
<p><b>補助項目五：發展原住民教育文化特色及充實設備器材</b>　<a href="allowance-05.htm" target="_blank">(補助項目說明)</a>
<p>
<font color=blue>※補助發展原住民文化特色及設備器材-申請補助經費：<?=$school_year;?>年度：<input type="text" size="6" name="s_total_money" value="<?=$s_total_money;?>" style="border:0px; text-align: right;" readonly >
元 (本列自動計算)</font><br />
<? if ($s_total_money_ny1 == 0) echo "<!--" ;?>
<font color=blue>※補助發展原住民文化特色及設備器材-申請補助經費：<?=($school_year+1);?>年度：<input type="text" size="6" name="s_total_money_ny1" value="<?=$s_total_money_ny1;?>" style="border:0px; text-align: right;" readonly >
元 (本列自動計算)</font><br />
<? if ($s_total_money_ny1 == 0) echo "-->" ;?>
<!--
<font color=blue>※補助發展原住民文化特色及設備器材-申請補助經費：<?=($school_year+2);?>年度：<input type="text" size="6" name="s_total_money_ny2" value="<?=$s_total_money_ny2;?>" style="border:0px; text-align: right;" readonly >
元 (本列自動計算)</font><br />
-->
說明：學生100人(含)以下最高補助12萬元，學生超過100人最高補助25萬元，分校分班最高補助8萬元，且<font color="red">「須發展原住民的相關特色」</font>；<font color="red">鐘點費(含補充保費)，金額會自動計入額外2%金額</font>。
<p>
※申請<input type="text" size="6" name="p_num" value="<?=$p_num;?>" style="border:0px; " readonly>項特色
<p>
<table border="0">
	<tr>
		<td nowrap="nowrap" style="text-align:right; vertical-align:top;">※特色一：</td>
		<td><input type="text" size="20" name="p1_name" value="<?=$p1_name;?>" style="border:0px; " readonly >(特色名稱)
			，參加學生人數<input type="text" size="6" name="s_p1_student" value="<?=$s_p1_student;?>" onChange="js:target_per('p1',this);" style="border:0px; " readonly>人
			，其中含目標學生<input type="text" size="6" name="s_p1_target" value="<?=$s_p1_target;?>" onChange="js:target_per('p1',this);" style="border:0px; " readonly>人
	...
	<tr>
		<td colspan="2">
			...
			本年度為第<input type="text" size="2" name="p1_ContinueYear" value="<?=$p1_ContinueYear;?>" style="border:0px; " readonly >年辦理。
		</td>
	</tr>
	<tr>
...
<?
	//顯示特色一 細項
	print_item($a5_table_name_item, 'p1_'.$school_year, $main_seq);
...
<?
	//顯示特色一 細項
	print_item($a5_table_name_item, 'p1_'.($school_year+1), $main_seq);       //j10407，修改為($school_year+1)，原105
...
<table border="0">
	<tr>
		...
		<td><input type="text" size="20" name="p2_name" value="<?=$p2_name;?>" style="border:0px; " readonly >(特色名稱)
			，參加學生人數<input type="text" size="6" name="s_p2_student" value="<?=$s_p2_student;?>" onChange="js:target_per('p2',this);" style="border:0px; " readonly>人
			，其中含目標學生<input type="text" size="6" name="s_p2_target" value="<?=$s_p2_target;?>" onChange="js:target_per('p2',this);" style="border:0px; " readonly>人
	...
	<tr>
		<td colspan="2">
			...
			本年度為第<input type="text" size="2" name="p2_ContinueYear" value="<?=$p2_ContinueYear;?>" style="border:0px; " readonly >年辦理。
		</td>
	</tr>
	<tr>
		<td nowrap="nowrap" colspan="2">
			<?=($school_year);?>年度：<font color=blue>經費<input type="text" size="6" name="s_p2_money" value="<?=$s_p2_money;?>" style="border:0px; text-align: right;" readonly>元
...
<?
	//顯示特色二 細項
	print_item($a5_table_name_item, 'p2_'.$school_year, $main_seq);
...
<?
	//顯示特色二 細項
	print_item($a5_table_name_item, 'p2_'.($school_year+1), $main_seq);              //j10407，修改為($school_year+1)，原105
	
?>
</table>
<? if (($s_total_money_ny1 == 0) || ($s_p2_money == 0))echo "-->" ;?> 
<p>
<p>

<p>
下載：<a href="/edu/modules/xSchoolForm/download/A0-1空白計畫範本.doc" target="_new">空白計畫範本</a><br />
說明：確認送出後，請於學校入口「上傳檔案專區」上傳「發展原住民教育文化特色及充實設備器材計畫」檔案。
<p>
<input type="hidden" name="main_seq" value="<?=$main_seq;?>">
<input type="hidden" name="school_year" value="<?=$school_year;?>">
<!-- input type="button" value="上一頁(不儲存)" onClick="history.go(-1)" -->
<input type="submit" name="button" value="確認申請並回上一頁" >

<INPUT TYPE="button" VALUE="修改經費內容" onclick="if(confirm('※重要提醒!!\n修改內容後，必須重新進入初審及複審作業，您確定要修改內容?')){location.href='school_write_a5_change.php'}">
...
<script language="JavaScript">
	function toCheck() 
	{
		...
		//金額檢核
		if(<?=$student;?> < 100 ){var maxMon = 120000; }else{var maxMon = 250000;}
		//alert(maxMon);
		if ( document.form.s_total_money.value > maxMon) 
		{
			errmsg += "申請金額超過可申請上限！\n";
			document.form.s_total_money.focus();
			flag = 0;
		}
		...
		*/
		...
		if(s_p_money != null)
		{
			...
			if(document.form.p2_name.value != "" && document.form.p2_name.value != "無")
			{
				...
				if( document.form.s_p2_money.value > maxMon) 
				{

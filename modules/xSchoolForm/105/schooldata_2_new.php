<?php
	...
	$sql = " select sd.account, sd.schooltype, sd.cityname, sd.district, sd.schoolname, sy.student ".
		   ...
		   "	  , sy_ly.teacher as teacher_ly ". //104年編制、調入、實缺、其他
		   ...
		   //j10407，新增前(103)年資料
		   "	  , sy_l2y.teacher as teacher_l2y ". //103年編制、調入、實缺、其他
		   "	  , sy_l2y.teacher_change_in as teacher_change_in_l2y ".
		   "	  , sy_l2y.teacher_change_lack as teacher_change_lack_l2y ".
		   "	  , sy_l2y.teacher_change_other as teacher_change_other_l2y ".	   
		   
		   "      , sd102.a135, sd102.a138, sd102.a141, sd102.a144 ". //102年編制、調入、實缺、其他
   
		   " from schooldata as sd left join schooldata_year as sy on sd.account = sy.account ".
		   
		   //去年資料的資料表
		   "                       left join schooldata_year as sy_ly on sd.account = sy_ly.account and sy_ly.school_year = '".($school_year - 1)."' ".       //104年資料

           //j10407，新增前(103)資料
		   "                       left join schooldata_year as sy_l2y on sd.account = sy_l2y.account and sy_l2y.school_year = '".($school_year - 2)."' ".    //10407，新增前(103)年資料	   
	...
	while($row = mysql_fetch_array($result))
	{
		...
		$student = $row['student'];
		$teacher_3years_total = $row['teacher_3years_total'];       //最近三年教師數
		$teacher = $row['teacher'];                                 //今年教師數
		$teacher_change_total = $row['teacher_change_total'];       //教師異動數3年合計
		$teacher_agent_total = $row['teacher_agent_total'];         //代理教師數3年合計
		$teacher_change_rate = $row['teacher_change_rate'];         //教師異動率%
		$teacher_agent_rate = $row['teacher_agent_rate'];           //代理教師率%
		$teacher_change_in = $row['teacher_change_in'];             //調入教師
		$teacher_change_lack = $row['teacher_change_lack'];         //實缺教師
		$teacher_change_other = $row['teacher_change_other'];       //其他教師
		
		//$teacher_last = ($row['teacher_ly'] == "")?"0":$row['teacher_ly'];          //104年教師編制
		
		//j10407，新增
		$teacher_ly = ($row['teacher_ly'] == "")?"0":$row['teacher_ly'];          //104年教師編制
		$teacher_change_in_ly = ($row['teacher_change_in_ly'] == "")?"0":$row['teacher_change_in_ly'];                     //調入教師
		$teacher_change_lack_ly = ($row['teacher_change_lack_ly'] == "")?"0":$row['teacher_change_lack_ly'];               //實缺教師
		$teacher_change_other_ly = ($row['teacher_change_other_ly'] == "")?"0":$row['teacher_change_other_ly'];            //其他教師

		/*
		//若無資料則代入去年的 
		$teacher_change_in_last = ($row['teacher_change_in_last'] == "")?$row['teacher_change_in_ly']:$row['teacher_change_in_last'];              //去年調入教師
		$teacher_change_lack_last = ($row['teacher_change_lack_last'] == "")?$row['teacher_change_lack_ly']:$row['teacher_change_lack_last'];      //去年實缺教師
		$teacher_change_other_last = ($row['teacher_change_other_last'] == "")?$row['teacher_change_other_ly']:$row['teacher_change_other_last'];  //去年其他教師
	  ...
	  //j10407，新增前(103)年資料
	    $teacher_last_l2y = ($row['teacher_l2y'] == "")?"0":$row['teacher_l2y'];    //j10407，新增前(103)年教師編制	  
		$teacher_change_in_last_l2y = ($row['teacher_change_in_l2y'] == "")?"0":$row['teacher_change_in_l2y'];              //前年調入教師
		$teacher_change_lack_last_l2y = ($row['teacher_change_lack_l2y'] == "")?"0":$row['teacher_change_lack_l2y'];      //前年實缺教師
		$teacher_change_other_last_l2y = ($row['teacher_change_other_l2y'] == "")?"0":$row['teacher_change_other_l2y'];  //前年其他教師	
		
		
		
		$teacher_last2 = ($row['a135'] == "")?"0":$row['a135']; //102年教師編制
		...
		{
			$teacher_3years_total = $teacher + $teacher_ly + $teacher_last_l2y;          //j10407，修正，原 $teacher_last+ $teacher_last2;
		}
		
		//教師異動人數 三年調入+三年實缺 第一次為空值須先計算
		if($teacher_change_total == '')
		{
			$teacher_change_total = $teacher_change_in + $teacher_change_in_ly + $teacher_change_in_last_l2y                    //j10407，修正，原$teacher_change_in_last+$teacher_change_in_last2
									+ $teacher_change_lack + $teacher_change_lack_ly + $teacher_change_lack_last_l2y;          //j10407，修正，原$teacher_change_lack_last+$teacher_change_lack_last2
		}
		
		//代理教師人數 三年實缺+三年其他 第一次為空值須先計算
		if($teacher_agent_total == '')
		{
			$teacher_agent_total = $teacher_change_other + $teacher_change_other_ly + $teacher_change_other_last_l2y           //j10407，修正，原$teacher_change_other_last +$teacher_change_other_last2
									+ $teacher_change_lack + $teacher_change_lack_ly + $teacher_change_lack_last_l2y;          //j10407，修正，原$teacher_change_lack_last+$teacher_change_lack_last2
		}
		...
		*/
		...
		//j10407，修正，原本是上面的，但要重新計算
		$teacher_3years_total = $teacher + $teacher_ly + $teacher_last_l2y;                                                 //j10407，修正，原+ $teacher_last2;
		$teacher_change_total = $teacher_change_in + $teacher_change_in_ly + $teacher_change_in_last_l2y                    //j10407，修正，原+$teacher_change_in_last2
									+ $teacher_change_lack + $teacher_change_lack_ly + $teacher_change_lack_last_l2y;
		$teacher_agent_total = $teacher_change_other + $teacher_change_other_ly + $teacher_change_other_last_l2y            //j10407，修正，原+$teacher_change_other_last2
									+ $teacher_change_lack + $teacher_change_lack_ly + $teacher_change_lack_last_l2y;
		$teacher_change_rate = number_format($teacher_change_total / $teacher_3years_total * 100, 2);		
		$teacher_agent_rate = number_format($teacher_agent_total / $teacher_3years_total * 100, 2);
...
<p><font color="blue"><strong>學校教師資料_105_new_sd2</strong></font>
...
<input type="text" size="3" name="tnumber3" value="<?=$teacher_last_l2y;?>" style="border:0px" readonly> 人 <em><sup>(註2)</sup></em><br />
※<?=($school_year-2);?>學年度全校教師編制人數：
<input type="text" size="3" name="tnumber2" value="<?=$teacher_ly;?>" style="border:0px" readonly> 人 <em><sup>(註2)</sup></em><br />
...
二、教師異動人數：<input type="text" size="2" name="different" value="<?=$teacher_change_total;?>" style="border:0px" readonly>人，
    ...
    <?//  echo  $teacher_change_in."+". $teacher_change_in_ly."+".$teacher_change_in_last_l2y."+".$teacher_change_lack."+".$teacher_change_lack_ly."+". $teacher_change_lack_last_l2y; ?>
...
　　調入教師<input type="text" size="3" name="ltnumber3" onChange="change_teacher()" value="<?=$teacher_change_in_last_l2y;?>" style="border:0px" readonly>人；                                       <!--j10407，修正，原 $teacher_change_in_last2    -->
實缺教師<input type="text" size="3" name="ltnumber6" onchange="change_teacher()" value="<?=$teacher_change_lack_last_l2y;?>" style="border:0px" readonly>人；                                         <!--j10407，修正，原 $teacher_change_lack_last2   -->
其他教師<input type="text" size="3" name="ltnumber9" onchange="change_teacher()" value="<?=$teacher_change_other_last_l2y;?>" style="border:0px" readonly>人 <em><sup>(註2)</sup></em><br />          <!--j10407，修正，原 $teacher_change_other_last2  -->
※<?=($school_year-2);?>學年度教師異動人數：<br />
　　調入教師<input type="text" size="3" name="ltnumber2" onChange="change_teacher()" value="<?=$teacher_change_in_ly;?>" />人；        <!--j10407，修正，原 $teacher_change_in_last    --> 
實缺教師<input type="text" size="3" name="ltnumber5" onchange="change_teacher()" value="<?=$teacher_change_lack_ly;?>" />人；          <!--j10407，修正，原 $teacher_change_lack_last    -->
其他教師<input type="text" size="3" name="ltnumber8" onchange="change_teacher()" value="<?=$teacher_change_other_ly;?>" />人<br />     <!--j10407，修正，原 $teacher_change_other_last    -->
...
<script language="JavaScript">
	...
	function toCheck() 
	{
		...
		if ( document.form.tnumber1.value >= 40 ) 
		{
			...
			{	
				show_errmsg = 1;
				document.form.page2_warning.value = "「104年全校教師編制人數與去年」比超過±10%，請填寫異動過大說明"            //j10407，修正年度
				
				if(document.form.page2_desc.value == "" ) //未填原因才顯示訊息
				{
					errmsg += '「104學年度全校教師編制人數」與「103學年度全校教師編制人數」不得超過±10%'+'﹙103年度全校教師40人以上才檢核﹚\n';    //j10407，修正年度
					flag = 0;
				}
			}
		}
		...
		{	
			...
			if(document.form.page2_desc.value == "" ) //未填原因才顯示訊息
			{	
				errmsg += '全校學生數：104學年度全校教師編制人數，比值不可＞20 或 ＜6 ﹙學生數100人以上才檢核﹚\n';             //j10407，修正年度
				flag = 0;
			}
		}
		...
		{	
			show_errmsg = 1;
			document.form.page2_warning.value = "「教師流動率」異動過大, 請說明原因"
		...
		{	
			show_errmsg = 1;
			document.form.page2_warning.value = "「實缺教師代理率」＞50%，異動過大, 請說明原因"

...
<form name="form" method="post" action="schooldata_3_finish_new.php" onSubmit="return toCheck();" onKeyDown="if(event.keyCode == 13) return false;">
<div id="print_content">
<p><font color="blue"><strong>目標學生資料_105_new_sd3</strong></font> 
<p>
<b>一、<?=$school_year;?>年目標學生數</b>
...
<table border="0">
	<tr>
		<td nowrap="nowrap">
			<?=$school_year-1;?>年目標學生數(不含僅具原住民身分者)：<?=$target_no_aboriginal_last?>
		</td>
	</tr>
	<tr>
		<td nowrap="nowrap" >
			<?=$school_year-1;?>年目標學生數(含僅原住民身分者)：<?=$target_aboriginal_last?>
		</td>
	</tr>
	<tr>
		<td nowrap="nowrap" >
			<?=$school_year-1;?>年原住民學生數：<?=$aboriginal_last?>
		</td>
	</tr>
	<tr>
		<td nowrap="nowrap" >
	...
	<tr>
		<td nowrap="nowrap" >
			...
			D.新住民子女學生數：<input type="text" size="6" name="immigrant" value="<?=$immigrant;?>"/>人<br />
			E.單(寄)親家庭學生數：<input type="text" size="6" name="single_parent" value="<?=$single_parent;?>"/>人<br />
			F.原住民學生數：<input type="text" size="6" name="aboriginal" value="<?=$aboriginal;?>"/>人   <?// echo $all_target;?>
			<p>
		</td>
	</tr>
	<tr>
	...
	<tr>
		<td nowrap="nowrap" >
			※<?=$school_year-2;?>學年度中途輟學學生數：<input type="text" size="6" name="lastyear_leaving" value="<?=$lastyear_leaving;?>"/>人<br />
			<font color=blue>　（從<?=$school_year-2;?>年9月1日至<?=$school_year-1;?>年6月30日之中輟學生數）</font>
			<p>
		</td>
	</tr>
</table>
...
<script language="JavaScript">
	...
	function toCheck() 
	{
		...
		//j10407,「目標學生數(含僅原住民身分者)」應均小於或等於 「A+B+C+D+E+F」、「目標學生數(不含僅具原住民身分者)」應均小於或等於 「A+B+C+D+E」
		/*	
		var y = <?=$low_income+$grandparenting+$over45years+$immigrant+$single_parent+$aboriginal;?>;
		var z = <?=$low_income+$grandparenting+$over45years+$immigrant+$single_parent;?>;
		//var z = parseInt(low_income)+parseInt(grandparenting)+parseInt(over45years)+parseInt(immigrant)+ parseInt(single_parent)+parseInt(aboriginal);
		//var sum = parseFloat(document.getElementById("text_1").value)+parseFloat(document.getElementById("text_2").value);
		//var sum = parseFloat(document.getElementById("low_income").value) + parseFloat(document.getElementById("grandparenting").value) + parseFloat(document.getElementById("over45years").value) parseFloat(document.getElementById("immigrant").value) + parseFloat(document.getElementById("single_parent").value) + parseFloat(document.getElementById("aboriginal").value);
		//var all_target = <? $all_target; ?>;		//parseInt
		
		if (document.form.target_aboriginal.value*1 > parseInt(y))
		{
				errmsg += '「目標學生數(含僅原住民身分者)」應均小於或等於 「A+B+C+D+E+F」！\n';
				flag = 0;		
		}
		
		if ( document.form.target_no_aboriginal.value*1 > parseInt(z)) 
		{
				errmsg += '「目標學生數(不含僅具原住民身分者)」應均小於或等於 「A+B+C+D+E」！\n';
				flag = 0;		
		}
		*/
		//------

...
<?
	...
	// 判斷是否符合指標4
	$per  = $lastyear_leaving / $laststudent;
	$p4_1 = ($per >= 0.03);
...
<? 
	$page_from = $_GET['page_from'];  /* 若參數為1=顯示關閉本頁，不顯示 */
	
	if($page_from == 1)
	{
		echo "<input type='button' value='關閉本頁' onClick='window.close()'>";
	}
	else
	{
		echo "<input type='button' value=' 下一步，前往「步驟2、申請補助經費」 ' style='height:30px;' onclick=location.href='school_select_allowance.php'> ";
		echo "<input type='button' value='回主選單' onclick=location.href='../school_index.php'>";
	}
?>

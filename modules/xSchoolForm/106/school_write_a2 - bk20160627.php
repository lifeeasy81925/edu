...
<body>
	...
	<form name="form" method="post" action="school_write_a2_finish.php" onSubmit="return toCheck();" onKeyDown="if(event.keyCode == 13) return false;">
		...
		● <a href="allowance-02.htm" target="_blank"><b><u>補助項目說明</u></b></a><p>
		...
		<script language="JavaScript">
			...
			function toCheck()
			{
				...
				{
					if(<?=$class_total;?> < <?=$keyClasses;?>)  // 小於 13 班(1～12 班)，不得超過 1 項特色
					{
						if(document.form.p_num.value > 1)
						{
							errmsg += '貴校班級數為 <?=$class_total;?> 班，不得申請超過 1 項特色。\n';
							document.form.p_num.focus();
							flag = 0;
						}
					}
					else  // 13 班以上，不得超過 2 項特色
					{
				...
				if(document.form.s_p1_money_ny1.value > 80000)
				{
					errmsg += '<?=$school_year+1;?>年特色一申請金額超過可申請上限！\n';
					flag = 0;
				}
				if(document.form.s_p1_money_ny2.value > 80000)
				{
				...
				{
					if(document.form.s_p1_money.value > 60000 || document.form.s_p1_money_ny1.value > 60000 || document.form.s_p1_money_ny2.value > 60000)
					{
				...
				if(<?=$class_total;?> >= <?=$keyClasses;?>)
				{
					...
					if(s_p_money > 0)  // 特色二有寫經費才開始檢查
					{
						...
						{
							errmsg += '<?=$school_year;?>特色二申請金額超過可申請上限！\n';
							flag = 0;
						}
						if(document.form.s_p2_money_ny1.value > 80000)
						{
							errmsg += '<?=$school_year+1;?>年特色二申請金額超過可申請上限！\n';
							flag = 0;
						}
						if(document.form.s_p2_money_ny2.value > 80000)
						{
							errmsg += '<?=$school_year+2;?>年特色二申請金額超過可申請上限！\n';
							flag = 0;
						}
						if(("<?=$schoolname;?>".indexOf("分校") != -1) || ("<?=$schoolname;?>".indexOf("分班") != -1))
						{
							if(document.form.s_p2_money.value > 60000 || document.form.s_p2_money_ny1.value > 60000 || document.form.s_p2_money_ny2.value > 60000)
							{
								errmsg += '分校分班每年最高補助 6 萬元。\n';
								flag = 0;
							}

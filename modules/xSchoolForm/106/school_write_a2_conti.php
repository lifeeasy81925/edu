...
<body>
	<form name="form" method="post" action="school_write_a2_finish.php" onKeyDown="if(event.keyCode == 13) return false;">
		...
		<table border="2" rules="rows" cellspacing="2" cellpadding="2">
			...
			<? print_item($a2_table_name_item, 'p1_'.$school_year, $main_seq);  // 顯示細項 ?>
		</table>
		<p>
		...
		<table>
			<tr>
				...
				<td align="right">
					<input type="button" value="修改經費內容" onclick="if(confirm('● 重要提醒！\n您目前是沿用已審核的經費概算表，今年度免初審。\n若要修改，只能修改經費，不得修改特色名稱，而且必須放棄先前已審核的經費，並重新進入初審及複審作業，您確定要修改內容嗎？')){location.href='school_write_a2_change.php'}">
				</td>
			</tr>
		</table>

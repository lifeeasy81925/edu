<?php
	...
	*/
	...
	{
		global $supplementary_premium_rate;
		...
		$sum_s_money = 0;  // ［106年度限定款］加總不含雜支的總額
		...
		while($row = mysql_fetch_array($result))
		{
			...
			if($category != '雜支')
			{
				...
				echo "<tr align='center'>";
					...
					// ═══════════════════════════════════════════════════════════════════════╗
					//［106年度限定款］因為補充保費調整，載入時重新計算補充保費。             ║ 
			                                                                               // ║ 
					if(strpos($category, "補充保費") > 0)                                  // ║ 
					{                                                                      // ║ 
						$s_money = round($price * $amount * $supplementary_premium_rate);  // ║ 
					}	                                                                   // ║ 
					// ═══════════════════════════════════════════════════════════════════════╝ 
				...
				if($subject == "經常門")  // 105.10.12 加入經常門才有算雜支
				{
					$sum_s_money += $s_money;  //［106年度限定款］加總不含雜支的總額
				}
			}
			else
			{
		...
		else  //［106年度限定款］雜支配合補充保費調整而調整
		{
			$outlay_s_money = round($sum_s_money * 0.06);
		}
...
<body>
	<form name="form" method="post" action="school_write_a2_finish.php" onSubmit="return toCheck();" onKeyDown="if(event.keyCode == 13) return false;">
		...
		<!-- 這個檔案是修正計畫，修正計畫需在第二年或第三年提出，所以不會有下下年度出現。 -->
	...
	<!-- 特色一與特色二的分隔線 -->
		...
		<script language="JavaScript">
			...
			function toCheck()
			{
				...
				var s_p2_money = parseInt(document.form.s_p2_money.value)
							  + parseInt(document.form.s_p2_money_ny1.value);

				if(s_p2_money > 0)  // 特色二有寫經費才開始檢查
				{
					// 空值、合理值檢查
				...
				/* 總金額檢查 */
				if(document.form.s_total_money.value > 160000)
				{
					errmsg += '<?=$school_year;?>年申請金額超過可申請上限！\n';
					flag = 0;
				}
				...
				if(document.form.s_total_money_ny1.value > 160000)
				{
					errmsg += '<?=$school_year+1;?>年申請金額超過可申請上限！\n';
					flag = 0;
				}
				else if(<?=$class_total;?> < <?=$keyClasses?> && document.form.s_total_money_ny1.value > 80000)
				{
					errmsg += '<?=$school_year;?>年申請金額超過可申請上限！\n';
					flag = 0;
				}
				if(("<?=$schoolname;?>".indexOf("分校") != -1) || ("<?=$schoolname;?>".indexOf("分班") != -1) || ("<?=$schoolname;?>".indexOf("分部") != -1))
				{
					if(document.form.s_p1_money.value > 60000 || document.form.s_p1_money_ny1.value > 60000)
					{

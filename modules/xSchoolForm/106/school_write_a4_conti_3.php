	...
	function print_item($allowance_table_name, $p, $main_seq)
	{
		...
		while($row = mysql_fetch_array($result))
		{
			...
			if($category != '雜支')
			{
				...
				if($subject == "經常門")  // 105.11.01 加入經常門才有算雜支
				{
	...
	function toCheck()
			{
				...
				else if(<?=$student;?> < <?=$keyStudent?> && document.form.s_total_money.value > 120000)
				{
					errmsg += '<?=$school_year;?>年申請金額超過可申請上限！\n';
					flag = 0;
				}				
				...
				else if(<?=$student;?> < <?=$keyStudent?> && document.form.s_total_money_ny1.value > 120000)
				{
					errmsg += '<?=$school_year;?>年申請金額超過可申請上限！\n';
					flag = 0;
				}
				...
				{
					if(document.form.s_p1_money.value > 80000 || document.form.s_p1_money_ny1.value > 80000)
					{
